...
class BLTEnhancedRAG(VoxSigilRAG):
    ...
    def search_sigils_by_similarity(self, query: str, num_results: int = 5,
                                   min_score_threshold: float = 0.0) -> List[Dict[str, Any]]:
        """
    ...
    def create_rag_context(self, query: str, num_sigils: int = 5, **kwargs) -> Tuple[str, List[Dict[str, Any]]]:
        ...
        context, retrieved_sigils = super().create_rag_context(query=query, num_sigils=num_sigils, **kwargs)
        
        # Apply BLT-specific optimizations if needed
        if self.enable_patch_compression:
            # Check if we need to compress
            max_context_chars = kwargs.get('max_context_chars_budget', self.default_max_context_chars)
            
            if len(context) > max_context_chars:
                # Apply patch-aware compression
                compressed_context, ratio = self.patch_compressor.compress(context)
                
                logger.debug(f"Applied BLT compression: {len(context)} → {len(compressed_context)} chars ({ratio:.2f} ratio)")
                ...
                try:
                    ...
                    if last_updated > threshold_time:
                        ...
                        if new_score > current_score:
                            ...
                            logger.debug(f"Applied BLT recent boost of {min_boost:.3f} to sigil '{sigil.get('sigil', 'N/A')}'")
    ...
    def _optimize_context_by_chars(self, sigils_for_context: List[Dict[str, Any]], 
                                  initial_detail_level: str,
                                  target_char_budget: int) -> Tuple[List[Dict[str, Any]], str]:
        """
        ...
        current_chars = sum(len(self.format_sigil_for_prompt(s, detail_level)) for s in optimized_sigils)
        
        if current_chars > target_char_budget and self.enable_patch_compression:
            logger.info(f"BLT optimization: Still over budget after basic optimization. Applying patch compression.")
    ...
    def enhanced_rag_process(self, 
                            ...
                            use_blt_encoding: bool = True) -> Tuple[str, List[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            logger.warning(f"Error using create_patches method: {e}. Using patch_encoder fallback instead.")
            ...
            patches = []
            chunk_size = len(effective_query) // len(entropy_scores) if entropy_scores else len(effective_query)
            for i, score in enumerate(entropy_scores):
                start_idx = i * chunk_size
                end_idx = min(start_idx + chunk_size, len(effective_query))
                content = effective_query[start_idx:end_idx] if start_idx < len(effective_query) else ""
        ...
        try:
            ...
            if self.enable_patch_compression and len(formatted_context) > max_context_chars:
                ...
                logger.info(f"Applied BLT compression: {len(formatted_context)} → {len(compressed_context)} chars ({ratio:.2f} ratio)")
    ...
    def format_sigil_for_prompt(self, sigil: Dict[str, Any], detail_level: str = "standard") -> str:
        ...
        if self.enable_patch_validation and '_blt_info' in sigil:
            ...
            if 'entropy' in blt_data:
                ...
                entropy_desc = "high" if entropy_val > 0.7 else "medium" if entropy_val > 0.4 else "low"
    ...
    def auto_fuse_related_sigils(self, base_sigils: List[Dict[str, Any]], max_additional: int = 3) -> List[Dict[str, Any]]:
                ...
                for other_sigil in all_system_sigils:
                    ...
                    try:
                        ...
                        if max_entropy == 0:  # Avoid division by zero
                            ...
                            entropy_similarity = 1.0 - abs(base_entropy - other_entropy) / max_entropy
                        ...
                        if entropy_similarity > 0.85:  # High entropy similarity threshold
                            ...
                            related_data['_fusion_reason'] = f"blt_entropy_match:{base_id}({entropy_similarity:.2f})"
                            related_data.setdefault('_similarity_score', 0.35 + (entropy_similarity * 0.1))
        ...
        if added_count > 0:
            logger.info(f"BLT fusion: Added {added_count} additional sigils based on BLT characteristics")
    ...
    def _validate_sigil_data(self, sigil_data: Dict[str, Any], file_path: str = None) -> bool:
        ...
        try:
            ...
            if not blt_valid and blt_issues:
                issue_str = "; ".join(issue.get('message', 'Unknown issue') for issue in blt_issues if 'message' in issue)
    ...
    def get_sigils(self, refresh=False):
        ...
        if refresh or not hasattr(self, "_loaded_sigils") or self._loaded_sigils is None:
            ...
            logger.info(f"Loaded {len(self._loaded_sigils)} sigils from {self.voxsigil_library_path}")
