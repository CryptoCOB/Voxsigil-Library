...
class VoxSigilConfig:
    ...
    def __init__(self):
        self.DEFAULT_VOXSİGİL_LIBRARY_PATH = Path(__file__).resolve().parent / "VoxSigil-Library"
        self.DEFAULT_EMBEDDING_MODEL = "all-MiniLM-L6-v2"
        self.DEFAULT_EMBEDDINGS_CACHE_PATH = Path(__file__).resolve().parent / "cache" / "embeddings_cache.npz"
...
try:
    ...
    logger.warning("Sentence Transformers or scikit-learn not available. Semantic features will be limited.")
...
def set_global_tokenizer(tokenizer_instance: Optional[Any], name: Optional[str] = None) -> None:
...
def count_tokens(text: str) -> int:
    ...
    if _global_tokenizer and hasattr(_global_tokenizer, 'encode'):
        try:
            ...
            logger.warning(f"Error using global tokenizer ({_tokenizer_name}): {e}. Falling back to char count.")
...
DEFAULT_SIGIL_SCHEMA = { # Copied from RAG, ensure it's used
    "type": "object",
    "properties": {
        "sigil": {"type": "string"}, "tag": {"type": "string"},
        "tags": {"type": ["array", "string"], "items": {"type": "string"}},
        "principle": {"type": "string"}, "usage": {"type": "object"},
        "prompt_template": {"type": "object"}, "relationships": {"type": "object"},
        "_source_file": {"type": "string"}, "_last_modified": {"type": "number"},
        "_similarity_score": {"type": "number"}, "_recency_boost_applied": {"type": "number"},
...
class VoxSigilRAG:
    def __init__(self, voxsigil_library_path: Optional[Path] = None, cache_enabled: bool = True,
                 ...
                 recency_boost_factor: float = 0.05, recency_max_days: int = 90,
                 default_max_context_chars: int = 8000):
        self.voxsigil_library_path = voxsigil_library_path or VS_CONFIG.DEFAULT_VOXSİGİL_LIBRARY_PATH
        ...
        if HAVE_SENTENCE_TRANSFORMERS:
            try:
                ...
                logger.error(f"[RAG] Error loading embedding model {embedding_model_name}: {e}")
    ...
    def _validate_sigil_data(self, sigil_data: Dict[str, Any], file_path: Path) -> bool:
        if not HAVE_JSONSCHEMA: return True
        try:
            validate(instance=sigil_data, schema=self.sigil_schema)
            return True
        except ValidationError as e:
            path_str = " -> ".join(map(str, e.path))
            logger.warning(f"[RAG] Schema validation failed for {file_path}: {e.message} (at path: '{path_str}')")
            return False
        except Exception as e_generic:
            logger.error(f"[RAG] Generic error during schema validation for {file_path}: {e_generic}")
            return False
    ...
    def _load_sigil_file(self, file_path: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not isinstance(sigil_data, dict): return None
            if not self._validate_sigil_data(sigil_data, file_path): return None
            
            sigil_data['_last_modified'] = file_mod_time
            sigil_data.setdefault('_source_file', str(file_path)) # NEW FEATURE 6 - ensure source
            # NEW FEATURE 6 - Check for other metadata
            for meta_key in ["creation_date", "version", "author"]:
                if meta_key not in sigil_data:
                    sigil_data.setdefault(meta_key, None) # Default to None if not present

            if self.cache_enabled: self._sigil_cache[cache_key] = sigil_data
            return sigil_data
        except Exception as e:
    ...
    def load_all_sigils(self, force_reload: bool = False) -> List[Dict[str, Any]]:
        ...
        sigil_dirs = [
            self.voxsigil_library_path / "core", self.voxsigil_library_path / "pglyph",
            self.voxsigil_library_path / "sigils", self.voxsigil_library_path / "examples",
            self.voxsigil_library_path / "constructs", self.voxsigil_library_path / "loader",
            self.voxsigil_library_path / "memory", self.voxsigil_library_path / "tags",
        ...
        loaded_sigils_list = [s for f in sigil_files if (s := self._load_sigil_file(f)) is not None]
        
        logger.info(f"[RAG] Successfully loaded {len(loaded_sigils_list)} sigils from {len(sigil_files)} files.")
    ...
    def format_sigil_for_prompt(self, sigil: Dict[str, Any], detail_level: str = "standard") -> str:
        ...
        all_tags = set()
        if 'tag' in sigil and sigil['tag']: all_tags.add(str(sigil['tag']))
        if 'tags' in sigil and sigil['tags']:
            tags_val = sigil['tags']
            if isinstance(tags_val, list): all_tags.update(str(t) for t in tags_val)
            elif isinstance(tags_val, str): all_tags.add(tags_val)
        if all_tags:
            output.append("Tags: " + ", ".join(f'"{t}"' for t in sorted(list(all_tags))))
        if 'principle' in sigil: output.append(f"Principle: \"{sigil['principle']}\"")
        if detail_level.lower() == "summary": return "\n".join(output)
        # ... (Standard and Full details as in original RAG)
        if 'usage' in sigil and isinstance(sigil['usage'], dict):
             if 'description' in sigil['usage']: output.append(f"Usage: \"{sigil['usage']['description']}\"")
        if '_source_file' in sigil : output.append(f"Source File: {Path(sigil['_source_file']).name}")
        # ... (Full detail logic)
        return "\n".join(output)
    ...
    def _apply_recency_boost(self, sigils_with_scores: List[Dict[str, Any]]) -> List[Dict[str, Any]]: # From RAG script
        # ... (content of _apply_recency_boost)
        if not self.recency_boost_factor > 0: return sigils_with_scores
    ...
    def _optimize_context_by_chars(self, sigils_for_context: List[Dict[str, Any]], initial_detail_level: str, target_char_budget: int) -> Tuple[List[Dict[str, Any]], str]: # From RAG script
        ...
        if not target_char_budget or not final_sigils: return final_sigils, current_detail_level
        ...
        current_detail_idx = detail_levels_ordered.index(current_detail_level) if current_detail_level in detail_levels_ordered else 1
    ...
    def _lightweight_retrieve_sigils(self, query: str, num_sigils: int, filter_tags: Optional[List[str]] = None, tag_operator: str = "OR") -> List[Dict[str, Any]]:
        logger.info(f"[RAG] Performing lightweight retrieval for query: '{query[:50]}...'")
        ...
        if filter_tags:
            ...
            for sigil in all_loaded_sigils:
                s_tags_set = set()
                if 'tag' in sigil and sigil['tag']: s_tags_set.add(str(sigil['tag']).lower())
                if 'tags' in sigil and sigil['tags']:
                    tags_v = sigil['tags']
                    if isinstance(tags_v, list): s_tags_set.update(str(t).lower() for t in tags_v)
                    elif isinstance(tags_v, str): s_tags_set.add(tags_v.lower())
                
                match = False
                if tag_operator.upper() == "AND":
                    if norm_filter_tags.issubset(s_tags_set): match = True
                else: # OR
                    if not norm_filter_tags.isdisjoint(s_tags_set): match = True
                if match: filtered_by_tags.append(sigil)
        ...
        for sigil in target_pool:
            score = 0.0
            text_to_search = (sigil.get('sigil', '') + " " + sigil.get('principle', '')).lower()
    ...
    def create_rag_context(self, num_sigils: int = 5, filter_tag: Optional[str] = None, 
                           ...
                           exclude_tags: Optional[List[str]] = None, exclude_sigil_ids: Optional[List[str]] = None,
                           augment_query_flag: bool = True, apply_recency_boost_flag: bool = True,
                           enable_context_optimizer: bool = False, max_context_chars_budget: Optional[int] = None,
        ...
        if not all_loaded_sigils_initial: return "", []
        ...
        if exclude_tags or exclude_sigil_ids:
            ...
            for sigil in current_sigils_pool:
                is_excluded = False
                if sigil.get('sigil','').lower() in norm_exclude_ids: is_excluded = True
                if not is_excluded and norm_exclude_tags:
                    s_tags_set = set() # ... collect sigil's tags ...
                    if 'tags' in sigil: s_tags_set.update(t.lower() for t in (sigil['tags'] if isinstance(sigil['tags'],list) else [sigil['tags']]))
                    if 'tag' in sigil: s_tags_set.add(str(sigil['tag']).lower())
                    if not norm_exclude_tags.isdisjoint(s_tags_set): is_excluded = True
                if not is_excluded: temp_pool.append(sigil)
            current_sigils_pool = temp_pool
        # Apply inclusion filters
        if filter_tag and not filter_tags: filter_tags = [filter_tag]
        if filter_tags and current_sigils_pool:
            ...
            for sigil in current_sigils_pool:
                ...
                if tag_operator.upper() == "AND":
                    ...
                    if not norm_filter_tags.isdisjoint(s_tags_set): match = True
                if match: temp_pool.append(sigil)
            current_sigils_pool = temp_pool

        sigils_with_scores: List[Dict[str, Any]] = []
        effective_query = query
        if query and augment_query_flag: effective_query = self._augment_query(query)

        if effective_query and retrieval_mode == "lightweight": # NEW FEATURE 2
            sigils_with_scores = self._lightweight_retrieve_sigils(effective_query, num_sigils * 2, filter_tags, tag_operator) # Get more to allow other sort/boost
            # Lightweight score is already in _similarity_score
        elif effective_query and retrieval_mode == "semantic" and HAVE_SENTENCE_TRANSFORMERS and self.embedding_model and current_sigils_pool:
            logger.info(f"[RAG] Semantic search with query: '{effective_query[:50]}...' on {len(current_sigils_pool)} candidates.")
            ...
            for sigil_data in current_sigils_pool:
                ...
                if sigil_embedding is not None:
                    similarity = np.dot(query_embedding, sigil_embedding) / (np.linalg.norm(query_embedding) * np.linalg.norm(sigil_embedding))
        ...
        elif current_sigils_pool: # No semantic/lightweight query, but sigils remain after filtering
            for sigil_data in current_sigils_pool:
                s_copy = sigil_data.copy()
        ...
        if enable_context_optimizer and final_retrieved_sigils:
            ...
            final_retrieved_sigils, current_detail_level = self._optimize_context_by_chars(final_retrieved_sigils, detail_level, char_budget)
    ...
    def inject_voxsigil_context(self, prompt: str, **kwargs) -> Tuple[str, List[Dict[str, Any]]]: # Use kwargs
        ...
        if 'prompt' in kwargs_for_create: del kwargs_for_create['prompt']

        sigil_context, retrieved_sigils = self.create_rag_context(query=search_query, **kwargs_for_create)
    ...
    def _get_sigil_text_for_embedding(self, sigil: Dict[str, Any]) -> str: # From RAG script
        # ... (content of _get_sigil_text_for_embedding, condensed)
        texts = []
        if sigil.get('sigil'): texts.append(f"Sigil: {sigil['sigil']}")
        # ... (tags, principle, usage as in original RAG) ...
        return "\n".join(texts)


    def _get_sigil_embedding(self, sigil: Dict[str, Any]) -> Optional[np.ndarray]: # From RAG script
        # (content from RAG, including robust cache key)
        if not HAVE_SENTENCE_TRANSFORMERS or not self.embedding_model: return None
        sigil_id = sigil.get('sigil')
        mod_time = sigil.get('_last_modified', 0)
        cache_key_base = f"id:{sigil_id}" if sigil_id else f"file:{sigil.get('_source_file', hashlib.md5(str(sigil).encode()).hexdigest())}"
        cache_key = f"{cache_key_base}::mod:{mod_time}::model:{self.embedding_model_name}"
        if self.cache_enabled and cache_key in self._embeddings_cache: return self._embeddings_cache[cache_key]
        text = self._get_sigil_text_for_embedding(sigil)
        if not text.strip(): return None
        try:
            embedding = self.embedding_model.encode(text)
            if self.cache_enabled: self._embeddings_cache[cache_key] = embedding
            return embedding
        except Exception as e:
    ...
    # For example:
    def _load_embeddings_cache(self): logger.debug("[RAG] _load_embeddings_cache called") # Placeholder
    def _save_embeddings_cache(self): logger.debug("[RAG] _save_embeddings_cache called") # Placeholder
    def precompute_all_embeddings(self, force_recompute: bool = False, batch_size: int = 32) -> int:
        logger.info(f"[RAG] Precomputing embeddings (force: {force_recompute}, batch: {batch_size})")
        # ... (actual implementation) ...
        return 0
    def _generate_rag_signature(self, retrieved_sigils: List[Dict[str,Any]]) -> str:
        if not retrieved_sigils: return "/* RAG Sig: None */"
...
class VoxSigilMiddleware:
    def __init__(self, voxsigil_rag_instance: Optional[VoxSigilRAG] = None,
        ...
        self.conversation_history: deque[Dict[str, Any]] = deque(maxlen=conversation_history_size)
        self.selected_sigils_history: Dict[int, List[Dict[str, Any]]] = {}
        self.turn_counter = 0
        self.rag_off_keywords = [kw.lower() for kw in rag_off_keywords] if rag_off_keywords else ["@@norag@@", "stop rag"]
        ...
        self.default_rag_params = { # For middleware specific defaults
            "num_sigils": 3, "min_score_threshold": 0.35, "detail_level": "standard",
            "include_explanations": True, "retrieval_mode": "semantic"
        }


    def _detect_intent(self, text: str) -> Optional[str]: # From Middleware
        # ... (content as in middleware)
        if not self.enable_intent_detection: return None
        text_lower = text.lower()
        if "explain" in text_lower or "what is" in text_lower: return "explain_concept"
        if "example" in text_lower: return "find_example"
        return None

    def _get_focused_history_for_rag_query(self, current_prompt: str) -> str: # From Middleware
        # ... (content as in middleware, condensed)
        if not self.conversation_history: return ""
        history_list = list(self.conversation_history)
        parts = []
        if len(history_list) > 0 and history_list[-1]['role'] == 'user': parts.append(f"Prev User: {history_list[-1]['content'][:100]}...")
        if len(history_list) > 1 and history_list[-2]['role'] == 'assistant': parts.append(f"Prev AI: {history_list[-2]['content'][:100]}...")
        return "\n".join(parts)

    def _parse_dynamic_rag_config(self, text: str) -> Tuple[str, Dict[str, Any]]: # From Middleware
        config: Dict[str, Any] = {}
        ...
        if match:
            try:
                ...
                logger.warning(f"[Middleware] Invalid JSON in @@voxsigil_config: {config_str}")
        return text, config

    def _determine_rag_strategy(self, user_prompt: str, current_rag_params: Dict[str, Any]) -> str: # NEW FEATURE 3
        ...
        if len(user_prompt.split()) < 3 or "quick answer" in user_prompt.lower():
            logger.info("[Middleware] Switching to lightweight RAG strategy due to short/simple prompt.")
            return "lightweight"
        if "complex analysis" in user_prompt.lower():
            logger.info("[Middleware] Ensuring semantic RAG strategy for complex query.")
    ...
    def preprocess_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not original_messages: return request

        # Add new user message to history
        last_msg = original_messages[-1]
        if last_msg.get('role') == 'user':
            if not self.conversation_history or self.conversation_history[-1] != last_msg:
                 self.conversation_history.append(last_msg.copy())
        
        user_message_content = last_msg.get('content', '') if last_msg.get('role') == 'user' else ''
        ...
        user_message_content, dynamic_config = self._parse_dynamic_rag_config(user_message_content)
        current_rag_params.update(dynamic_config)
        if user_message_index != -1: original_messages[user_message_index]['content'] = user_message_content # Update if cleaned
        
        # NEW FEATURE 3: Dynamic RAG Strategy
        current_rag_params["retrieval_mode"] = self._determine_rag_strategy(user_message_content, current_rag_params)

        # Conditional RAG
        if any(keyword in user_message_content.lower() for keyword in self.rag_off_keywords) or \
           len(user_message_content.split()) < self.min_prompt_len_for_rag or \
           current_rag_params["retrieval_mode"] == "off":
        ...
        rag_query_text = f"{history_context_for_rag}\n\nCURRENT QUERY: {user_message_content}" if history_context_for_rag else user_message_content
        ...
        if detected_intent and detected_intent in self.intent_to_tags_map:
            ...
            current_rag_params["filter_tags"] = list(set((current_rag_params.get("filter_tags") or []) + intent_tags))
        ...
        if rag_cache_key in self._rag_cache:
            ...
            try:
                enhanced_prompt, retrieved_sigils = self.voxsigil_rag.inject_voxsigil_context(
                    prompt=user_message_content, query=rag_query_text, **current_rag_params
                )
                self._rag_cache[rag_cache_key] = (enhanced_prompt, retrieved_sigils)
            except Exception as e:
                logger.error(f"[Middleware] Error during RAG injection: {e}. Using original prompt.", exc_info=True)
                enhanced_prompt = user_message_content 
                retrieved_sigils = []

        self.selected_sigils_history[current_turn_id] = retrieved_sigils
        if user_message_index != -1: original_messages[user_message_index]['content'] = enhanced_prompt
    ...
    def postprocess_response(self, response: Dict[str, Any], scoring_enabled: bool = True) -> Dict[str, Any]:
        ...
        response['voxsigil_metadata']['retrieved_sigils'] = [
            {'sigil': s.get('sigil','N/A'), 'score':s.get('_similarity_score',0)} for s in retrieved_sigils
        ]
    ...
    def wrap_llm_api(self, llm_api_call: Callable[..., Any]) -> Callable[..., Any]: # From Middleware
        def wrapped_llm_api(*args: Any, **kwargs: Any) -> Any:
            request_dict = kwargs.get('request') if 'request' in kwargs and isinstance(kwargs.get('request'), dict) else (args[0] if args and isinstance(args[0], dict) and 'messages' in args[0] else None)
            if not request_dict: return llm_api_call(*args, **kwargs)
            
            processed_request = self.preprocess_request(request_dict.copy())
            
            if 'request' in kwargs and isinstance(kwargs.get('request'), dict): kwargs['request'] = processed_request
            elif args and isinstance(args[0], dict) : args = (processed_request,) + args[1:]
                
            raw_response = llm_api_call(*args, **kwargs)
            return self.postprocess_response(raw_response.copy()) if isinstance(raw_response, dict) else raw_response
...
class VoxSigilResponseEvaluator:
    def __init__(self, voxsigil_rag_instance: Optional[VoxSigilRAG] = None,
        ...
        if HAVE_SENTENCE_TRANSFORMERS and self.embedding_model_name:
            try:
                logger.info(f"[Evaluator] Loading embedding model: {embedding_model_name}")
                self.embedding_model = SentenceTransformer(embedding_model_name)
            except Exception as e:
                logger.error(f"[Evaluator] Error loading embedding model: {e}")
                # No hard fail, semantic similarity will be skipped

        self.default_score_weights = default_score_weights or { # Default profile weights
            'tag_inclusion': 0.20, 'tag_paraphrasing': 0.10, 'principle_adherence': 0.25,
            'sigil_mentions': 0.10, 'structural_integrity': 0.05, 'lexical_overlap': 0.05,
            'semantic_similarity': 0.15, 'contradiction_penalty_factor': 0.5 # Added from other evaluator
        }
        self.evaluation_profiles = evaluation_profiles or {"default": self.default_score_weights.copy()}
    ...
    def set_active_evaluation_profile(self, profile_name: str) -> bool: # NEW FEATURE 4
        if profile_name in self.evaluation_profiles:
            ...
            logger.info(f"[Evaluator] Active evaluation profile set to: '{profile_name}'")
            return True
    ...
    def get_current_score_weights(self) -> Dict[str, float]: # NEW FEATURE 4
        return self.evaluation_profiles.get(self.active_profile_name, self.default_score_weights)

    def _get_response_embedding_eval(self, response_text: str) -> Optional[np.ndarray]: # Renamed for clarity
        if self._current_response_embedding_cache is not None: return self._current_response_embedding_cache
        if self.embedding_model:
            try:
                self._current_response_embedding_cache = self.embedding_model.encode(response_text)
                return self._current_response_embedding_cache
            except Exception as e: logger.warning(f"[Evaluator] Embedding failed for response: {e}")
        return None

    # Evaluation sub-methods from Enhanced Evaluator, adapted
    def _evaluate_tag_inclusion(self, response_lower: str, relevant_sigils: List[Dict[str, Any]], expected_tags: Optional[List[str]]) -> Tuple[float, List[Dict[str, Any]]]:
        ...
        if not all_expected_tags:
            for item in relevant_sigils:
                ...
                if 'tag' in sigil and sigil['tag']: all_expected_tags.add(str(sigil['tag']).lower())
                if 'tags' in sigil and sigil['tags']:
                    tags_val = sigil['tags']
                    if isinstance(tags_val, list): all_expected_tags.update(str(t).lower() for t in tags_val)
                    elif isinstance(tags_val, str): all_expected_tags.add(tags_val.lower())
        
        if not all_expected_tags: return 0.0, []

        matched_count = 0
        for tag_lower in all_expected_tags:
            if tag_lower in response_lower:
                matched_count +=1
                tag_matches_details.append({'tag': tag_lower, 'status': 'matched_explicitly'})
            else:
    ...
    def _evaluate_principle_adherence(self, response_text: str, response_lower: str, relevant_sigils: List[Dict[str, Any]]) -> Tuple[float, List[Dict[str, Any]]]:
        ...
        response_embedding_eval = self._get_response_embedding_eval(response_text) # Use evaluator's specific embed method

        for item in relevant_sigils:
            sigil = item.get('sigil', {})
            sigil_id = sigil.get('sigil', 'unknown')
            principle_text = sigil.get('principle','')
            if not principle_text: continue

            current_sigil_score = 0.0
            method_used = "keyword"
            current_match_detail = {'sigil': sigil_id, 'score':0.0, 'method': method_used, 'matches': []}
            ...
            if self.embedding_model and response_embedding_eval is not None:
                try:
                    ...
                    if cosine_similarity: # Check if sklearn's cosine_similarity is imported
                        sim = cosine_similarity([response_embedding_eval], [principle_embedding])[0][0]
                        current_sigil_score = (sim + 1) / 2 # Normalize
                        method_used = "semantic"
                        current_match_detail['matches'].append({'type': 'semantic', 'value': round(current_sigil_score,3)})
                except Exception: # Fallback on error
                    pass # Keyword check will run

            if method_used == "keyword": # Fallback or primary
                key_phrases = [p.strip().lower() for p in principle_text.split('.') if len(p.strip()) > 5][:3]
                if key_phrases:
                    matched_phrases = sum(1 for kp in key_phrases if kp in response_lower)
                    current_sigil_score = matched_phrases / len(key_phrases)
                    current_match_detail['matches'].append({'type': 'keyword', 'value': round(current_sigil_score,3)})
        ...
        overall_score = np.mean(all_sigil_principle_scores) if all_sigil_principle_scores else 0.0
    ...
    def _evaluate_sigil_mentions(self, response_lower: str, relevant_sigils: List[Dict[str, Any]]) -> Tuple[float, List[Dict[str, Any]]]:
        mention_details = []
        mention_count = 0
        for item in relevant_sigils:
            sigil = item.get('sigil',{})
    ...
    def _evaluate_structural_integrity(self, response: str) -> Tuple[float, List[Dict[str, Any]]]: # Basic version
        patterns = []
        score = 0.0
        if re.search(r'\n\s*\d+\.\s+', response): patterns.append({'type':'numbered_list', 'count': len(re.findall(r'\n\s*\d+\.\s+',response))}); score = max(score, 0.5)
        if re.search(r'\n\s*[\*\-]\s+', response): patterns.append({'type':'bullet_list', 'count': len(re.findall(r'\n\s*[\*\-]\s+',response))}); score = max(score, 0.5)
        if len(patterns) > 1: score = 0.8
        if not patterns and len(response.split('\n')) > 5 : score = 0.2 # Some structure if long and multiline
        return score, patterns

    def _evaluate_lexical_overlap(self, response_lower: str, relevant_sigils: List[Dict[str, Any]]) -> float:
        sigil_texts_combined = " ".join(
            item.get('sigil',{}).get('principle','') + " " + item.get('sigil',{}).get('sigil','')
            for item in relevant_sigils
        ).lower()
        if not sigil_texts_combined: return 0.0
        ...
        if not sigil_words: return 0.0
        
        overlap = len(sigil_words.intersection(response_words))
        return overlap / len(sigil_words)

    def _evaluate_semantic_similarity(self, response: str, relevant_sigils: List[Dict[str, Any]]) -> Tuple[float, Dict[str, float]]:
        if not self.embedding_model or not cosine_similarity: return 0.0, {}
        response_embedding = self._get_response_embedding_eval(response)
        if response_embedding is None: return 0.0, {}
        ...
        for item in relevant_sigils:
            sigil = item.get('sigil',{})
            sigil_id = sigil.get('sigil','unknown')
            sigil_text = sigil.get('principle','') + " " + " ".join(sigil.get('tags',[])) # Combine principle and tags for semantics
            if not sigil_text.strip(): continue
            ...
            try:
                ...
                logger.debug(f"[Evaluator] Semantic sim error for sigil {sigil_id}: {e}")
    ...
    def evaluate_response_enhanced(self, response_text: str, query: str,
                                   expected_tags: Optional[List[str]] = None,
                                   relevant_sigils_override: Optional[List[Dict[str, Any]]] = None, # Renamed from relevant_sigils
        ...
        if relevant_sigils_override is None:
            _, retrieved_items = self.voxsigil_rag.create_rag_context(query=query, num_sigils=5) # Default 5
        ...
        scores: Dict[str, float] = {
            'tag_inclusion': 0.0, 'tag_paraphrasing': 0.0, 'principle_adherence': 0.0,
            'sigil_mentions': 0.0, 'structural_integrity': 0.0, 'lexical_overlap': 0.0,
            'semantic_similarity': 0.0, 'contradiction_score': 0.0, # From other evaluator version
            'total_score': 0.0
        }
        details: Dict[str, Any] = {'per_sigil_scores': {}} # Changed from sigil_scores to per_sigil_scores
        ...
        if not actual_expected_tags and current_relevant_sigils:
             for item in current_relevant_sigils:
                ...
                if 'tag' in item and item['tag']: actual_expected_tags.append(str(item['tag']))
                if 'tags' in item and item['tags']:
                    val = item['tags']
                    if isinstance(val, list): actual_expected_tags.extend(str(t) for t in val)
                    elif isinstance(val, str): actual_expected_tags.append(val)
        actual_expected_tags = list(set(actual_expected_tags)) # Unique

        # --- Call evaluation sub-methods ---
        scores['tag_inclusion'], details['tag_matches_report'] = self._evaluate_tag_inclusion(response_lower, current_relevant_sigils, actual_expected_tags)
        scores['principle_adherence'], details['principle_adherence_report'] = self._evaluate_principle_adherence(response_text, response_lower, current_relevant_sigils)
        scores['sigil_mentions'], details['sigil_mentions_report'] = self._evaluate_sigil_mentions(response_lower, current_relevant_sigils)
        scores['structural_integrity'], details['structural_patterns_report'] = self._evaluate_structural_integrity(response_text)
        scores['lexical_overlap'] = self._evaluate_lexical_overlap(response_lower, current_relevant_sigils)
        
        if self.embedding_model:
            scores['semantic_similarity'], details['semantic_similarity_per_sigil'] = self._evaluate_semantic_similarity(response_text, current_relevant_sigils)
            ...
            if actual_expected_tags:
                ...
                response_embedding_eval = self._get_response_embedding_eval(response_text)
                if response_embedding_eval is not None and self.embedding_model and cosine_similarity:
                    try:
                        tag_embeddings = self.embedding_model.encode(tag_concepts)
                        sims = cosine_similarity([response_embedding_eval], tag_embeddings)[0]
                        scores['tag_paraphrasing'] = float(np.mean(sims)) if len(sims) > 0 else 0.0
                    except Exception as e: logger.debug(f"Tag paraphrasing embedding failed: {e}")
            else: scores['tag_paraphrasing'] = 0.0
        else: # Fallback for tag_paraphrasing
            scores['tag_paraphrasing'] = scores['lexical_overlap'] * 0.5 # Simple fallback
        ...
        for sigil_item in current_relevant_sigils:
            ...
            contradiction = self._calculate_contradiction_for_sigil(response_lower, sigil_item) # From other evaluator
            max_contradiction = max(max_contradiction, contradiction)
            if sigil_id not in details['per_sigil_scores']: details['per_sigil_scores'][sigil_id] = {}
        ...
        scores['total_score'] = sum(
            ...
            for metric in current_weights if metric != 'contradiction_penalty_factor'
        )
        # Apply contradiction penalty
        penalty = scores.get('contradiction_score', 0.0) * current_weights.get('contradiction_penalty_factor', 0.5)
        scores['total_score'] = max(0.0, scores['total_score'] - penalty)

        # Round all scores
        for k in scores: scores[k] = round(scores[k], 4)

        return {'scores': scores, 'details': details} if detailed_report else {'scores': scores}

    def generate_visual_diff(self, responses_map: Dict[str, str], query_for_rag: str, title: str = "VoxSigil Response Comparison") -> str:
        # (Method content from Enhanced Evaluator, using self.evaluate_response_enhanced)
        html_parts = [f"<!DOCTYPE html><html><head><meta charset='UTF-8'><title>{title}</title><style>{VS_CONFIG.HTML_DIFF_STYLES}</style></head><body><h1>{title}</h1><div class='container'>"]
        
        # Fetch relevant_sigils once for the query
        # The RAG call now expects kwargs correctly
        _, common_relevant_sigils = self.voxsigil_rag.create_rag_context(query=query_for_rag, num_sigils=5)


        for variant_name, response_text in responses_map.items():
            if not response_text: continue
            evaluation = self.evaluate_response_enhanced(response_text, query_for_rag, relevant_sigils_override=common_relevant_sigils, detailed_report=True)
            scores = evaluation.get("scores", {})
            # Highlight response (basic version for now)
            formatted_response = html.escape(response_text).replace("\n", "<br>") 
            
            html_parts.append(f"<div class='variant'><div class='variant-header'><h2>{variant_name} (Score: {scores.get('total_score',0.0):.2f})</h2></div>")
            html_parts.append(f"<div class='content'>{formatted_response}</div>")
            html_parts.append("<div class='metrics'><h3 class='metric-header'>Metrics</h3><table><tr><th>Metric</th><th>Score</th></tr>")
            for metric, score_val in sorted(scores.items()):
                color_class = "high" if score_val >= 0.7 else ("medium" if score_val >=0.4 else "low")
                if metric == "contradiction_score" and score_val > 0.1: color_class = "low" # Higher contradiction is bad
                elif metric == "contradiction_score": color_class = "high"

                html_parts.append(f"<tr><td>{metric}</td><td><span class='score {color_class}'>{score_val:.3f}</span></td></tr>")
            html_parts.append("</table></div></div>") # Close metrics and variant
            
        html_parts.append("</div></body></html>")
        return "".join(html_parts)

    def save_comparison_to_file(self, responses_map: Dict[str, str], query_for_rag:str, output_path: Union[str, Path], title: str = "VoxSigil Response Comparison") -> str:
        ...
        with open(output_p, 'w', encoding='utf-8') as f: f.write(html_content)
        logger.info(f"[Evaluator] Comparison saved to {output_p}")
        return str(output_p)
    
    def evaluate_batch_enhanced(self, prompt_response_pairs: List[Dict[str, Any]], 
        ...
        for i, pair in enumerate(prompt_response_pairs):
            ...
            if not prompt or not response:
                logger.warning(f"[Evaluator] Skipping batch item {eval_id}: missing prompt or response.")
                all_results.append({'id': eval_id, 'error': 'Missing prompt or response', 'scores': {}})
                continue
            ...
            logger.info(f"[Evaluator] Evaluating batch item: {eval_id} (Query: {prompt[:30]}...)")
            evaluation = self.evaluate_response_enhanced(response, prompt, relevant_sigils_override=relevant_sigils, detailed_report=True)
            
            current_result = {
                'id': eval_id,
                'prompt': prompt,
                'response_snippet': response[:100] + "...", # Snippet for brevity in main results list
                'scores': evaluation.get('scores',{}),
        ...
        batch_report = {'results': all_results, 'summary_statistics': summary_statistics}
        if output_file:
...
if __name__ == "__main__":
    ...
    sigil_alpha_data = {
        "sigil": "α_Concept", "principle": "Alpha defines foundational concepts with clarity.",
        "tags": ["concept", "foundation", "alpha_tag"], "version": "1.0", "author": "System"
    }
    sigil_beta_data = {
        "sigil": "β_Example", "principle": "Beta provides practical examples for illustration.",
        "tags": ["example", "practical", "beta_tag"], "version": "1.1", "author": "System"
    }
    with open(core_sigils_path / "alpha.voxsigil", 'w') as f: yaml.dump(sigil_alpha_data, f)
    with open(core_sigils_path / "beta.yaml", 'w') as f: yaml.dump(sigil_beta_data, f)
    ...
    # --- Middleware Demo ---
    logger.info("\n--- Middleware Demo ---")
    vs_middleware = VoxSigilMiddleware(voxsigil_rag_instance=vs_rag, conversation_history_size=3)
    
    def mock_llm_api_call(request: Dict[str, Any]) -> Dict[str, Any]:
        user_prompt_content = request['messages'][-1]['content']
        logger.info(f"  [Mock LLM] Received enhanced prompt (first 150 chars): '{user_prompt_content[:150]}...'")
        response_content = f"Acknowledging enhanced prompt about concepts. The α_Concept sigil is indeed about clarity. Examples use β_Example."
        if "@@voxsigil_config" in user_prompt_content: # Check if the original user prompt had it
    ...
    test_request_mw = {
        'messages': [{'role': 'user', 'content': 'Explain foundational concepts with examples. @@voxsigil_config:{"num_sigils":1}@@'}]
    }
    response_mw = wrapped_api(request=test_request_mw.copy())
    print(f"  Middleware Demo Response (metadata): {json.dumps(response_mw.get('voxsigil_metadata'), indent=2)}")
    print(f"  Middleware Demo Response (content): {response_mw['choices'][0]['message']['content']}")
    ...
    eval_profiles = {
        ...
        "strict_principle": {**vs_rag.default_rag_params, 'principle_adherence': 0.7, 'tag_inclusion': 0.1},
        "tag_focused": {**vs_rag.default_rag_params, 'tag_inclusion': 0.6, 'principle_adherence': 0.2}
    }

    vs_evaluator = VoxSigilResponseEvaluator(voxsigil_rag_instance=vs_rag, evaluation_profiles=eval_profiles)
    vs_evaluator.set_active_evaluation_profile("strict_principle") # Use a custom profile
    ...
    _, relevant_sigils_for_eval = vs_rag.create_rag_context(query=eval_query, num_sigils=2)
    
    evaluation_good = vs_evaluator.evaluate_response_enhanced(
        eval_response_good, eval_query, relevant_sigils_override=relevant_sigils_for_eval
    )
    print(f"  Evaluation for GOOD response (profile: {vs_evaluator.active_profile_name}):")
    ...
    print(f"    Principle Adherence: {evaluation_good['scores'].get('principle_adherence')}")
    ...
    responses_for_diff = {
        ...
        "LLM Simulated Response from Middleware": response_mw['choices'][0]['message']['content']
    }
    diff_output_path = Path("voxsigil_response_comparison.html")
    vs_evaluator.save_comparison_to_file(responses_for_diff, eval_query, diff_output_path, title="Demo VoxSigil Response Comparison")


    # --- Batch Evaluation Demo (Evaluator) ---
    logger.info("\n--- Batch Evaluation Demo ---")
    batch_eval_data = [
        {'id': 'good_eval', 'query': eval_query, 'response': eval_response_good, 'relevant_sigils': relevant_sigils_for_eval},
        {'id': 'poor_eval', 'query': eval_query, 'response': eval_response_poor, 'relevant_sigils': relevant_sigils_for_eval}, # Re-use relevant sigils
        {'id': 'middleware_eval', 'query': test_request_mw['messages'][0]['content'], 
         'response': response_mw['choices'][0]['message']['content'],
         'relevant_sigils': response_mw.get('voxsigil_metadata',{}).get('retrieved_sigils',[]) # Use what middleware retrieved
        },
    ]
    ...
    try:
        ...
        logger.info(f"    '{text_sample}' -> Tokens: {count_tokens(text_sample)} (using {_tokenizer_name})")
    except ImportError:
        logger.warning("    tiktoken not installed. Skipping global tokenizer demo.")
        logger.info(f"    '{text_sample}' -> Est. Tokens (chars/4): {count_tokens(text_sample)}")
