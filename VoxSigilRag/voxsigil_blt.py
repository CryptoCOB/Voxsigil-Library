...
class VoxSigilRAG:  # Placeholder for standard RAG
    ...
    def create_rag_context(
        ...
        # Use the embedding to initialise a deterministic RNG – this way `emb`
        # directly influences the mock-retrieval and is no longer “unused”.
        rng = np.random.default_rng(int(emb[0] * 1e6) & 0xFFFFFFFF)
    ...
    def retrieve_with_embedding(
        ...
        s = [
            {
                "id": f"std_emb_doc_{i}",
                "content": f"Std (pre-emb) content {i} for '{query_text[:20]}'",
                "score": np.random.rand(),
            }
            for i in range(num_sigils)
        ]
        return "STD CONTEXT (PRE-EMBEDDED):\n" + "\n".join(d["content"] for d in s), s
...
class PatchAwareValidator:  # Stub
    ...
    def validate_structure(self, text: str) -> Tuple[bool, List[Dict]]:
        ...
        try:
            ...
            # Log for debugging
            logger.debug(
                f"PatchAwareValidator: Structure validation - avg_entropy={avg_entropy:.4f}"
            )
...
class PatchAwareCompressor:  # Stub
    def __init__(self, entropy_threshold=0.5):
        logger.info("PatchAwareCompressor (stub) initialized.")

    def compress(self, text: str) -> Tuple[str, float]:
        if len(text) > 100:
            return text[: len(text) // 2] + "...", 0.5  # Dummy compression
        return text, 1.0
...
class BLTMiddlewareConfig(BaseSettings):
    ...
    enable_patch_validation: bool = Field(
        True, description="Enable BLT-based patch validation of sigils/context."
    )
    enable_patch_compression: bool = Field(
        False, description="Enable BLT-based patch compression of context/query."
    )
    compression_max_size: Optional[int] = Field(
        2048, description="Max size for content before compression (if enabled)."
    )
    ...
    max_semantic_cache_size: int = Field(
        100, description="Maximum number of entries in the semantic cache."
    )
...
class VoxSigilMiddleware:
    def __init__(
        ...
        **kwargs,
        ...
        self._standard_rag_instance = (
            voxsigil_rag_instance if voxsigil_rag_instance else VoxSigilRAG()
        )
    ...
    def _enhance_request_with_context(
        self, request: Dict[str, Any], context: str, context_source: str = "Hybrid"
    ) -> Dict[str, Any]:
        messages = request.get("messages", [])
    ...
    ) -> Callable[..., Any]:  # Identical to stub
...
class BLTEntropyRouter:  # Feature 1 & 2 & 8
    def __init__(self, config: BLTMiddlewareConfig):
        ...
        logger.info(
            f"BLTEntropyRouter initialized: threshold={config.entropy_threshold}, fallback_rag={config.entropy_router_fallback}"
        )
    ...
    def route(self, text: str) -> Tuple[str, Optional[List[str]], List[float]]:
        ...
        try:
            ...
            if len(entropy_scores) <= 10:
                ...
                logger.debug(
                    f"Entropy scores (sample of 10): {[round(e, 4) for e in entropy_scores[:10]]}, ... (total: {len(entropy_scores)})"
                )
            ...
            logger.error(
                f"Entropy routing failed: {e}. Using fallback_rag: {self.config.entropy_router_fallback}",
                exc_info=True,
            )
            return self.config.entropy_router_fallback, None, [0.5]
...
class BLTDynamicExecutionBudgeter:
    ...
    def allocate_budget(self, method: str, avg_entropy: float, length: int) -> float:
        budget = self.base_budget * (
            (1.0 - 0.4 * avg_entropy)
            if "blt" in method
            else (1.0 + self.entropy_multiplier * avg_entropy)
        )
...
class BLTEnhancedMiddleware(VoxSigilMiddleware):  # Inherits from VoxSigilMiddleware
    ...
    def __init__(
        ...
        **kwargs,
    ):  # Allows passing parent params directly
        ...
        self.patch_validator = PatchAwareValidator(
            entropy_threshold=self.config.entropy_threshold + 0.1
        )
        ...
        logger.info(
            f"BLTEnhancedMiddleware initialized with Hybrid capabilities. Cache TTL: {self.config.cache_ttl_seconds}s."
        )
        if self.config.enable_semantic_cache:
            logger.info(
                f"Semantic caching enabled with similarity threshold: {self.config.semantic_similarity_threshold}"
            )
        if self.config.enable_recency_boost:
            logger.info(
                f"Recency boosting enabled with factor: {self.config.recency_boost_factor}, max days: {self.config.recency_max_days}"
            )
    ...
    def blt_patch_encoder(
        ...
        if self._patch_encoder_instance is None:
            ...
            base_model_for_patch_encoder = (
                ...
                if hasattr(self._standard_rag_instance, "embedding_model")
                else None
            )
    ...
    def _clean_expired_cache_entries(self):  # Identical to previous
        ...
        expired = [
            k
            for k, (*_, ts) in self._rag_context_cache.items()
            if current_time - ts > self.config.cache_ttl_seconds
        ]
        for k in expired:
            del self._rag_context_cache[k]
        if expired:
            logger.info(
    ...
    def _get_hybrid_rag_context(
        ...
        if self.config.enable_semantic_cache and self.semantic_cache:
            ...
            if semantic_cache_result:
                ...
                logger.info(
                    f"Semantic cache hit with similarity {similarity:.4f} for query: '{query[:30]}...'"
                )
        ...
        try:
            if route_decision == "blt_rag":  # Low entropy path
                logger.debug(
                    f"Query '{query[:30]}...' routed to BLT RAG (entropy: {avg_entropy:.2f})"
                )
                ...
                logger.debug(
                    f"Query '{query[:30]}...' routed to Standard RAG (entropy: {avg_entropy:.2f})"
                )
            ...
            logger.error(
                f"Error in RAG path '{actual_method_used}' for query '{query[:30]}...': {e}",
                exc_info=True,
            )
            ...
            if actual_method_used.startswith("blt"):
                ...
                logger.warning(
                    "Standard RAG failed. Attempting fallback to BLT RAG (if applicable)."
                )
                try:
                    blt_embedding = self.blt_patch_encoder.encode(query)
                    context_str, sigils = (
    ...
    def _apply_blt_specific_enhancements(
        self, request: Dict[str, Any], context_str: str, sigils: List[Dict]
        ...
        if self.config.enable_patch_compression:
            ...
            if user_query and (
                not self.config.compression_max_size
                or len(user_query) > self.config.compression_max_size
            ):
            ...
            if final_context and (
                not self.config.compression_max_size
                or len(final_context) > self.config.compression_max_size
            ):
    ...
    def preprocess_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(query) >= 3:
            ...
            if self.config.enable_semantic_cache and self.semantic_cache:
                ...
                if semantic_result:
                    ...
                    logger.info(
                        f"Semantic cache HIT for query (similarity: {similarity:.4f}). Using cached RAG context. Route: {route_method}"
                    )
                    cache_hit = True
                else:
            ...
            if not cache_hit:
                ...
                if cached_entry:
                    ...
                    if current_monotonic_time - _ts <= self.config.cache_ttl_seconds:
                        logger.info(
                            f"Exact string cache HIT for query (key: {cache_key[:30]}...). Using cached RAG context. Route: {_route}"
                        )
                        context_str, sigils, route_method = _ctx, _sgls, _route
                        self._rag_context_cache[cache_key] = (
                            _ctx,
                            _sgls,
                            _route,
                        ...
                        logger.info(
                            f"Exact string cache STALE for query (key: {cache_key[:30]}...). Re-computing."
                        )
        ...
        enhanced_request, final_context_str = self._apply_blt_specific_enhancements(
            request, context_str, sigils
        )
...
def compute_true_hybrid_embedding(
    ...
    logger.debug(
        f"Computing true hybrid embedding for: '{text[:30]}...' with BLT weight: {blt_weight}"
    )
    try:
        std_emb = standard_encoder_model.encode(text)
        blt_emb = blt_patch_encoder_model.encode(
            text
        )  # Ensure dimensions match if target_dim is provided, or if they differ.
        # This is a placeholder for actual dimension alignment logic (padding/truncation/projection)
        if target_dim:
            std_emb = (
                std_emb[:target_dim]
                if len(std_emb) > target_dim
                else np.pad(std_emb, (0, target_dim - len(std_emb)))
            )
            blt_emb = (
                blt_emb[:target_dim]
                if len(blt_emb) > target_dim
                else np.pad(blt_emb, (0, target_dim - len(blt_emb)))
            )
        elif std_emb.shape != blt_emb.shape:
            logger.warning(
                f"Std emb shape {std_emb.shape} != BLT emb shape {blt_emb.shape}. Attempting resize to min dim."
            )
...
class ByteLatentTransformerEncoder:
    ...
    def __init__(self, base_embedding_model=None, patch_size=64, max_patches=16):
        ...
        logger.info(
            f"ByteLatentTransformerEncoder initialized with patch_size={patch_size}, max_patches={max_patches}"
        )
    ...
    def encode(self, text: str) -> np.ndarray:
        ...
        if self.base_embedding_model is not None:
            try:
                ...
                logger.warning(
                    f"Error using base embedding model: {e}. Falling back to default BLT encoding."
                )
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Test configuration: {test_conf_obj.model_dump_json(indent=2) if IS_PYDANTIC_AVAILABLE else vars(test_conf_obj)}"
    )
    ...
    test_requests_data = [
        ...
        },  # High entropy expected
        {
            "messages": [
                {
                    "role": "user",
                    "content": "<query type='sigils'><filter field='id'>S123</filter></query>",
                }
            ]
        },  # Low entropy
        {
            "messages": [
                {
                    "role": "user",
                    "content": "Tell me about {code_block: 'function foo() { return 1; }'}",
                }
            ]
        },  # Mixed
        {
        ...
        },  # Test cache
        {
            "messages": [
                {
                    "role": "user",
                    "content": "A very long query" * 20
                    + "that should trigger compression if enabled and above threshold.",
                }
            ]
        },  # Test compression
    ]
    ...
    for i, req_data_item in enumerate(test_requests_data):
        ...
        final_query = blt_middleware._extract_query_from_request(
            processed_req
    ...
    patch_encoder_for_util = SigilPatchEncoder(
        entropy_threshold=0.4, use_blt=True
    )  # Get a patch encoder
    ...
    if hybrid_emb_test is not None:
        print(
            f"True Hybrid Embedding computed, shape: {hybrid_emb_test.shape}, first 3: {hybrid_emb_test[:3]}"
        )
    else:
