...
try:
    ...
    print("Pydantic (pydantic, pydantic-settings) is not installed. Using basic config.")
...
class HybridMiddlewareConfig(BaseSettings):
    ...
    entropy_threshold: float = Field(0.25, description="Threshold for determining high vs low entropy. Inputs below this are routed to 'patch_based'.")
    blt_hybrid_weight: float = Field(0.7, description="Weight factor for BLT in hybrid embeddings (used by hybrid_embedding utility).")
    entropy_router_fallback: str = Field("token_based", description="Default path if entropy calculation fails ('patch_based' or 'token_based').")
    cache_ttl_seconds: int = Field(300, description="Time-to-live for cached RAG contexts in seconds.")
...
class VoxSigilRAG:
    """Placeholder for standard VoxSigil RAG component."""
    def __init__(self):
        logger.info("Standard VoxSigilRAG initialized (stub).")
        # Use top-level MockEmbeddingModel for multiprocessing compatibility
        self.embedding_model = MockEmbeddingModel()

    def _compute_text_embedding(self, text: str) -> np.ndarray:
        logger.debug(f"StandardRAG: Computing embedding for: '{text[:50]}...'")
        return self.embedding_model.encode(text)

    def create_rag_context(self, query: str, num_sigils: int = 5, **kwargs) -> Tuple[str, List[Dict[str, Any]]]:
        logger.info(f"StandardRAG: Creating context for query: '{query[:50]}...' with {num_sigils} sigils.")
        # Mock implementation
        embedding = self._compute_text_embedding(query) # For similarity search
        sigils = [{"id": f"std_doc_{i}", "content": f"Standard relevant document {i} for '{query[:20]}'", "score": np.random.rand()} for i in range(num_sigils)]
        context_str = "\n".join([s['content'] for s in sigils])
        return f"STANDARD RAG CONTEXT:\n{context_str}", sigils

class ByteLatentTransformerEncoder:
    """Placeholder for BLT encoder."""
    def __init__(self, base_embedding_model=None, patch_size=64, max_patches=16):
        ...
        logger.info(f"ByteLatentTransformerEncoder initialized with patch_size={patch_size}, max_patches={max_patches} (hybrid_blt.py)")

    def encode(self, text: str) -> np.ndarray:
        logger.debug(f"BLTEncoder: Encoding text: '{text[:50]}...'")
        # Simple hash-based deterministic "embedding"
        h = hashlib.sha256(f"blt_{text}".encode()).digest() # Add prefix to differentiate
        return np.frombuffer(h, dtype=np.float32)[:128] # Use first 128 floats
    ...
    def create_patches(self, text: str) -> List[Any]:
        ...
        if not validated_text:
            logger.debug("Empty text after validation in create_patches")
            return []
        ...
        patches = []
        # Safely convert to bytes with error handling
        try:
            text_bytes = validated_text.encode('utf-8', errors='replace')
            text_len = len(text_bytes)
        except Exception as e:
            logger.error(f"Error encoding text to bytes: {e}")
            # Fallback to creating at least one patch with the original text
            patch = Patch(validated_text, 0, len(validated_text), 0.5)  # Default entropy
            return [patch]
        
        if text_len == 0:
            logger.debug("Zero-length text_bytes in create_patches")
            return []
        ...
        for i in range(0, text_len, self.patch_size):
            ...
            try:
                ...
                logger.warning(f"Error calculating entropy for chunk at position {i}: {e}")
                entropy = 0.5  # Default fallback entropy
            ...
            try:
                ...
                logger.warning(f"Error creating patch at position {i}: {e}")
                continue
        ...
        if not patches and validated_text:
            logger.debug("No patches created, adding fallback patch")
            patch = Patch(validated_text[:min(len(validated_text), self.patch_size)], 
                          0, min(len(validated_text), self.patch_size), 0.5)
            patches = [patch]
    ...
    def calculate_similarity(self, embedding1: np.ndarray, embedding2: np.ndarray) -> float:
        ...
        if embedding1 is None or embedding2 is None:
            logger.warning("Received None embedding in calculate_similarity")
            return 0.0
            
        if not isinstance(embedding1, np.ndarray) or not isinstance(embedding2, np.ndarray):
            logger.warning(f"Invalid embedding types: {type(embedding1)}, {type(embedding2)}")
            return 0.0
            
        if embedding1.size == 0 or embedding2.size == 0:
            logger.warning("Received empty embedding in calculate_similarity")
            return 0.0
            
        if embedding1.shape != embedding2.shape:
            logger.warning(f"Embedding shape mismatch: {embedding1.shape} vs {embedding2.shape}")
            # Try to reshape or pad if sizes are different
...
class BLTEnhancedRAG(VoxSigilRAG): # Inherits for simplicity in stub
    """Placeholder for BLT-enhanced VoxSigil RAG component."""
    def __init__(self, entropy_threshold: float, blt_hybrid_weight: float):
        super().__init__() # Not strictly needed if overriding all methods
        self.entropy_threshold = entropy_threshold
        self.blt_hybrid_weight = blt_hybrid_weight
        self.blt_encoder = ByteLatentTransformerEncoder()
        logger.info(f"BLTEnhancedRAG initialized with threshold={entropy_threshold}, weight={blt_hybrid_weight} (stub).")

    def _compute_text_embedding(self, text: str) -> np.ndarray:
        logger.debug(f"BLTRAG: Computing BLT embedding for: '{text[:50]}...'")
        return self.blt_encoder.encode(text) # Use the BLT encoder

    def create_rag_context(self, query: str, num_sigils: int = 5, **kwargs) -> Tuple[str, List[Dict[str, Any]]]:
        logger.info(f"BLTRAG: Creating context for query: '{query[:50]}...' with {num_sigils} sigils.")
        # Mock implementation using BLT specific logic potentially
        embedding = self._compute_text_embedding(query) # For similarity search
        sigils = [{"id": f"blt_doc_{i}", "content": f"BLT relevant document {i} for '{query[:20]}'", "score": np.random.rand()} for i in range(num_sigils)]
        context_str = "\n".join([s['content'] for s in sigils])
...
class EntropyRouter:
    ...
    def __init__(self, config: HybridMiddlewareConfig):
        self.config = config
        self.patch_encoder = SigilPatchEncoder() # This would be your actual entropy model/analyzer
        logger.info(f"EntropyRouter initialized with threshold: {self.config.entropy_threshold}, fallback: {self.config.entropy_router_fallback}")
    ...
    def route(self, text: str) -> Tuple[str, Optional[List[str]], List[float]]:
        ...
        if not text:
            ...
            return self.config.entropy_router_fallback, None, [0.5] # Default to medium entropy for empty

        try:
            patches, entropy_scores = self.patch_encoder.analyze_entropy(text)
            
            if not entropy_scores: # If analyze_entropy returns empty scores for some reason
                logger.warning(f"Entropy calculation returned no scores for text: '{text[:50]}...'. Applying heuristic.")
                # Fallback heuristic if primary entropy model fails to give scores
                if any(c in text for c in ['<', '>', '{', '}', '[', ']']) and len(text) < 200: # More likely structured
                    avg_entropy = 0.15
                elif len(text) < 50 and ' ' not in text: # Short, no spaces, might be an identifier
                    avg_entropy = 0.2
            ...
            logger.info(f"Text avg_entropy: {avg_entropy:.4f} (threshold: {self.config.entropy_threshold}) for query: '{text[:30]}...'")
            
            if avg_entropy < self.config.entropy_threshold:
                return "patch_based", patches, entropy_scores # Low entropy, likely structured -> BLT/Patch
            else:
                return "token_based", None, entropy_scores # High entropy, likely natural language -> Standard/Token
        except Exception as e:
            logger.error(f"Entropy calculation/routing failed: {e}. Using fallback path: {self.config.entropy_router_fallback}", exc_info=True)
            return self.config.entropy_router_fallback, None, [0.5] # Default to medium entropy on error
...
class HybridProcessor:
    ...
    def __init__(self, config: HybridMiddlewareConfig):
        ...
        logger.info(f"HybridProcessor initialized.")
    ...
    def compute_embedding(self, text: str) -> Dict[str, Any]:
        ...
        try:
            if route == "patch_based":
                embedding = self.blt_rag._compute_text_embedding(text) # Uses BLT encoder
                method = "blt"
            else: # "token_based" or fallback
                embedding = self.standard_rag._compute_text_embedding(text) # Uses standard encoder
                method = "token"
        except Exception as e:
            logger.error(f"Error computing embedding via {method} path: {e}. Falling back to standard.", exc_info=True)
            embedding = self.standard_rag._compute_text_embedding(text) # Fallback to standard
    ...
    def get_rag_context_and_route(self, query: str, num_sigils: int = 5, **kwargs) -> Tuple[str, List[Dict[str, Any]], str]:
        """
        Determines route and creates RAG context using the appropriate method.
        """
        route, _, entropy_scores = self.router.route(query) # Patches and scores for logging/metrics if needed
        avg_entropy = sum(entropy_scores) / len(entropy_scores) if entropy_scores else 0.5
        ...
        try:
            if route == "patch_based":
                ...
                logger.info(f"Routing query to Standard RAG (entropy: {avg_entropy:.2f})")
                context_str, sigils_list = self.standard_rag.create_rag_context(
                    query=query, num_sigils=num_sigils, **kwargs
                )
        except Exception as e:
            logger.error(f"Error creating RAG context via {route} path: {e}. Attempting fallback.", exc_info=True)
            ...
            if route == "patch_based":
                ...
                try:
                    context_str, sigils_list = self.standard_rag.create_rag_context(query=query, num_sigils=num_sigils, **kwargs)
                ...
                logger.warning("Standard RAG failed. Falling back to BLT RAG (less common fallback).")
                try:
                    context_str, sigils_list = self.blt_rag.create_rag_context(query=query, num_sigils=num_sigils, **kwargs)
...
class DynamicExecutionBudgeter: # More conceptual in this script
    ...
    def allocate_budget(self, method: str, avg_entropy: float, text_length: int) -> float:
        ...
        if "blt" in method.lower(): # Patch-based or BLT
            ...
            budget *= (1.0 + self.entropy_multiplier * avg_entropy) # Higher entropy, more budget
        
        length_factor = max(0.5, min(2.0, text_length / 500.0)) # Scale around 500 chars
        budget *= length_factor
        logger.debug(f"Allocated budget: {budget:.2f} for method={method}, entropy={avg_entropy:.2f}, len={text_length}")
        return budget
...
class HybridMiddleware:
    ...
    def __init__(self, config: HybridMiddlewareConfig):
        self.config = config
        logger.info(f"Initializing HybridMiddleware with config.")
        self.processor = HybridProcessor(config)
        self.budgeter = DynamicExecutionBudgeter() # Using default budgeter params
        
        self._context_cache: Dict[str, Tuple[str, List[Dict[str, Any]], str, float]] = {} # key: (context, sigils, route, timestamp)
    ...
    def _initialize_voxsigil_components(self):
        ...
        try:
            ...
            if hasattr(self.processor, 'standard_rag') and self.processor.standard_rag:
                ...
                logger.warning("VoxSigil RAG component not available in processor. Limited functionality available.")
    ...
    def _normalize_sigil_relationships_format(self):
        ...
        """
        if not self.voxsigil_rag or not hasattr(self.voxsigil_rag, '_loaded_sigils') or not self.voxsigil_rag._loaded_sigils:
            # Try to load sigils if available
            if hasattr(self.voxsigil_rag, 'load_all_sigils'):
                self.voxsigil_rag.load_all_sigils(force_reload=False)
            else:
                logger.warning("No ability to load sigils. Skipping relationship format normalization.")
                return
            
        if not hasattr(self.voxsigil_rag, '_loaded_sigils') or not self.voxsigil_rag._loaded_sigils:
    ...
    def format_sigil_for_context(self, sigil: Dict[str, Any], detail_level: str = "standard", include_explanation: bool = False) -> str:
        """
        ...
        if detail_level.lower() in ("detailed", "full"):
            ...
            if detail_level.lower() == "full" and 'prompt_template' in sigil and isinstance(sigil['prompt_template'], dict):
    ...
    def format_sigils_for_context(self, sigils: List[Dict[str, Any]], detail_level: str = "standard", include_explanations: bool = False) -> str:
        """
        ...
        output_sections.append(f"--- End VoxSigil Context ---")
        
        return "\n\n".join(output_sections)
    
    def _get_cache_key(self, query: str) -> str:
        """Create a cache key from the query, normalizing it."""
        normalized_query = ' '.join(query.lower().strip().split()) # Lowercase, strip, normalize whitespace
    ...
    def _clean_expired_cache_entries(self) -> None:
        ...
        expired_keys = [
            key for key, (_, _, _, timestamp) in self._context_cache.items()
    ...
    def _extract_query_from_messages(self, messages: List[Dict[str, Any]]) -> Optional[str]:
        ...
        logger.warning(f"Could not extract a valid query string from messages: {messages}")
        return None

    def _enhance_messages_with_context(self, messages: List[Dict[str, Any]], context: str) -> List[Dict[str, Any]]:
        ...
        # Prepend to system message or add a new one
        # This strategy can be tuned based on how your LLM best uses context.
        ...
        if enhanced_messages and enhanced_messages[0].get("role") == "system":
            ...
            enhanced_messages[0]["content"] = f"{system_message_content}\n\n{original_system_content}"
    ...
    def __call__(self, model_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(messages, list) or not messages:
            logger.warning("No messages found or invalid format in model_input. Skipping VoxSigil enhancement.")
            return model_input # Passthrough if no valid messages

        query = self._extract_query_from_messages(messages)
        if not query:
            logger.warning("No query extractable from messages. Skipping VoxSigil enhancement.")
            return model_input
        ...
        if cached_entry:
            ...
            if current_monotonic_time - timestamp <= self.config.cache_ttl_seconds:
                logger.info(f"Cache HIT for query (key: {cache_key[:30]}...). Using cached context from route: {cached_route_method}")
                context_str, sigils_list, route_method = cached_context_str, cached_sigils_list, cached_route_method
                # Update timestamp to refresh TTL (Least Recently Used semantic for cache hit)
                self._context_cache[cache_key] = (context_str, sigils_list, route_method, current_monotonic_time)
                cache_hit = True
            else:
                logger.info(f"Cache STALE for query (key: {cache_key[:30]}...). Re-computing.")
                del self._context_cache[cache_key] # Remove stale entry

        if not cache_hit:
            logger.info(f"Cache MISS for query (key: {cache_key[:30]}...). Processing query.")
            try:
                # Determine route and get RAG context
                context_str, sigils_list, route_method = self.processor.get_rag_context_and_route(query, num_sigils=5) # num_sigils configurable
                
                # Cache the new result
                self._context_cache[cache_key] = (context_str, sigils_list, route_method, current_monotonic_time)
                logger.info(f"Successfully processed and cached query. Route: {route_method}")
            except Exception as e:
                logger.critical(f"Critical error during RAG context creation for query '{query[:50]}...': {e}", exc_info=True)
        ...
        if not cache_hit:
            ...
            if entropy_scores_for_budget:
                avg_entropy_for_budget = sum(entropy_scores_for_budget) / len(entropy_scores_for_budget)
        ...
        log_metadata = {
            "request_id": hashlib.md5(query.encode()).hexdigest()[:8], # Example request ID
    ...
    def _validate_sigil_data(self, sigil_data: Dict[str, Any], file_path: str = None) -> bool:
        ...
        try:
            ...
            logger.warning("jsonschema not available. Sigil schema validation will be skipped.")
            return True
        ...
        try:
            ...
            logger.warning(f"Schema validation failed for {file_path or 'sigil'}: {e.message} (at path: '{path_str}')")
            return False
        except Exception as e_generic: 
            logger.error(f"Generic error during schema validation for {file_path or 'sigil'}: {e_generic}")
            return False

    def _apply_recency_boost(self, sigils_with_scores: List[Dict[str, Any]], recency_boost_factor: float = 0.05, recency_max_days: int = 90) -> List[Dict[str, Any]]:
        """
    ...
    def auto_fuse_related_sigils(self, base_sigils: List[Dict[str, Any]], max_additional: int = 3) -> List[Dict[str, Any]]:
        """
        ...
        if not hasattr(self, 'voxsigil_rag') or not self.voxsigil_rag:
            logger.warning("VoxSigil RAG component not available. Cannot auto-fuse related sigils.")
            return base_sigils
    ...
    def _optimize_context_by_chars(
        ...
        try:
            ...
            current_detail_idx = 2  # Default to 'standard' if initial_detail_level is unknown
    ...
    def enhanced_rag_process(self, 
                        ...
                        max_fusion_sigils: int = 3) -> Tuple[str, List[Dict[str, Any]]]:
        """
        ...
        """
        if not hasattr(self, 'voxsigil_rag') or not self.voxsigil_rag:
            logger.warning("VoxSigil RAG component not available. Cannot perform enhanced RAG processing.")
            return "", []
        ...
        try:
            ...
            if route == "patch_based" and hasattr(self.processor, 'blt_rag'):
                ...
                logger.info(f"Using BLT-enhanced retrieval (entropy: {avg_entropy:.2f})")
                ...
                context_str, retrieved_sigils = self.processor.blt_rag.create_rag_context(
        ...
        if auto_fuse_related and retrieved_sigils:
            retrieved_sigils = self.auto_fuse_related_sigils(
                retrieved_sigils, 
                max_additional=max_fusion_sigils
            )
...
def hybrid_embedding_utility(
    ...
    """
    Computes a hybrid embedding, potentially blending BLT and standard based on entropy.
    This is a simplified version focusing on direct embedding, not full RAG.
    ...
    Returns:
        Tuple of (embedding_vector, method_used, average_entropy_score)
    """
    logger.info(f"Utility: hybrid_embedding for text: '{text[:30]}...'")
    router = EntropyRouter(config) # Independent router instance for this utility
    
    # Stubs for embedding models - in a real app, these would be shared or passed in
    standard_encoder = VoxSigilRAG().embedding_model 
    ...
    try:
        # For very clear routing decisions, use the direct path
        # Using a slightly wider margin around the threshold for "clear" decision
        if avg_entropy < config.entropy_threshold * 0.8: # Clearly low entropy
            logger.debug("Utility: Routing to BLT embedding (low entropy).")
            embedding = blt_encoder.encode(text)
            method_used = "blt_direct"
        elif avg_entropy > config.entropy_threshold * 1.2: # Clearly high entropy
            logger.debug("Utility: Routing to Standard embedding (high entropy).")
            embedding = standard_encoder.encode(text)
            method_used = "standard_direct"
        else: # Borderline entropy - true hybrid weighted combination
            logger.debug("Utility: Borderline entropy, computing true hybrid embedding.")
            blt_emb = blt_encoder.encode(text)
            std_emb = standard_encoder.encode(text)
            
            # Normalize embeddings before combining (important for consistent scaling)
            blt_norm = blt_emb / (np.linalg.norm(blt_emb) + 1e-9) # Add epsilon for stability
            std_norm = std_emb / (np.linalg.norm(std_norm) + 1e-9)
            
            # Weighted combination
            # Weight can be fixed (blt_hybrid_weight) or dynamic based on how close to threshold
            # Using fixed weight from config here for simplicity
            hybrid_emb = (config.blt_hybrid_weight * blt_norm) + \
                         ((1 - config.blt_hybrid_weight) * std_norm)
            
            embedding = hybrid_emb / (np.linalg.norm(hybrid_emb) + 1e-9) # Normalize final
            method_used = "hybrid_weighted"
            
    except Exception as e:
        logger.error(f"Utility: Error in hybrid_embedding: {e}. Falling back to standard.", exc_info=True)
        try:
            embedding = standard_encoder.encode(text)
            method_used = "standard_fallback"
        except Exception as fallback_e:
            logger.critical(f"Utility: Standard embedding fallback also failed: {fallback_e}", exc_info=True)
            embedding = None # Could return zeros or raise
            method_used = "failed"
...
def entropy_router(text, config=None):
    """
    Route text to appropriate processing path based on entropy.
    ...
    # Determine route
    if avg_entropy < config.entropy_threshold:
        return "patch_based", avg_entropy
    else:
        return "token_based", avg_entropy

def hybrid_embedding(text, token_embedding, patch_embedding, config=None):
    """
    Create a hybrid embedding combining token-based and patch-based embeddings.
    
    Args:
        text: Input text
        token_embedding: Standard token-based embedding
        patch_embedding: BLT patch-based embedding
        config: Optional configuration (uses default if None)
    ...
    route, entropy = entropy_router(text, config)
    
    # Calculate weights based on entropy and config
    if route == "patch_based":
        # Lower entropy = more weight to BLT
        blt_weight = config.blt_hybrid_weight
    else:
        # Higher entropy = more weight to tokens
        blt_weight = 1.0 - config.blt_hybrid_weight
        
    # Combine embeddings
    token_weight = 1.0 - blt_weight
    combined = (token_embedding * token_weight) + (patch_embedding * blt_weight)
...
if __name__ == "__main__":
    ...
    # Initialize with global APP_CONFIG (or create a specific one for testing)
    # For testing, you might want to override some config values
    test_config_overrides = {}
    if not IS_PYDANTIC_AVAILABLE: # If Pydantic is not installed, use basic class
        test_config = HybridMiddlewareConfig(**{
            "entropy_threshold":0.3, "log_level": "DEBUG", "cache_ttl_seconds": 60
        })
    else: # Use Pydantic with potential overrides
         test_config = HybridMiddlewareConfig(
             entropy_threshold=0.3, log_level="DEBUG", cache_ttl_seconds=60, 
             **test_config_overrides
        )
    
    # --- Test HybridMiddleware (RAG pipeline) ---
    print("\n--- Testing HybridMiddleware (RAG Processing) ---")
    middleware = HybridMiddleware(config=test_config)

    test_llm_requests = [
        {
            "model": "test_llm_model_alpha",
            "messages": [{"role": "user", "content": "Explain how symbolic reasoning works in LLMs."}],
            "temperature": 0.5
        },
        {
            "model": "test_llm_model_beta",
            "messages": [{"role": "system", "content": "You are a helpful assistant."},
                         {"role": "user", "content": "{\"sigil_id\": \"S001\", \"type\": \"metadata_query\", \"fields\": [\"name\", \"description\"]}"}],
        },
        {
            "model": "test_llm_model_gamma",
            "messages": [{"role": "user", "content": "Using VoxSigil with <tag>BLT components</tag>, how can we implement entropy-based compression for text containing <code>my_function(a,b)</code>?"}],
        },
        { # Test cache
            "model": "test_llm_model_alpha", # Same model
            "messages": [{"role": "user", "content": "Explain how symbolic reasoning works in LLMs."}], # Identical query
            "temperature": 0.5
        },
         { # Test similar query for cache normalization
            "model": "test_llm_model_alpha",
            "messages": [{"role": "user", "content": "  explain how symbolic reasoning works in llms.  "}], # Similar query
            "temperature": 0.5
        },
    ]

    for i, req_data in enumerate(test_llm_requests):
        print(f"\n--- Processing LLM Request {i+1} ---")
        query_preview = req_data["messages"][-1]["content"][:60] + "..."
        print(f"Original Query Preview: {query_preview}")
        
        start_req_time = time.monotonic()
        enhanced_req_data = middleware(req_data.copy()) # Pass a copy
        end_req_time = time.monotonic()
        
        print(f" Middleware processing time: {(end_req_time - start_req_time)*1000:.2f} ms")
        print(f" Enhanced Messages (System context preview):")
        if enhanced_req_data["messages"] and enhanced_req_data["messages"][0]["role"] == "system":
            print(f"  {enhanced_req_data['messages'][0]['content'][:200]}...")
        else:
            print("  No system message or context added.")
        
        metadata = enhanced_req_data.get("voxsigil_metadata", {})
        print(f" VoxSigil Metadata: {metadata.get('route_method_used', 'N/A')}, Cache: {metadata.get('cache_hit', 'N/A')}")

    print(f"\n--- Middleware Stats after {len(test_llm_requests)} requests ---")
    print(middleware.get_stats())


    # --- Test hybrid_embedding_utility ---
    print("\n\n--- Testing hybrid_embedding_utility ---")
    utility_queries = [
        "This is a piece of natural language text, likely high entropy.",
        "<config><setting name='foo'>bar</setting><value>123</value></config>", # Structured, low entropy
        "A short phrase." # Borderline example
    ]
    for query in utility_queries:
        print(f"\n--- Utility: Embedding query: '{query[:50]}...'")
        emb_start_time = time.monotonic()
        embedding, method, entropy = hybrid_embedding_utility(query, config=test_config)
        emb_proc_time = time.monotonic() - emb_start_time
        
        print(f" Method Used: {method}")
        print(f" Avg Entropy: {entropy:.4f}")
        print(f" Processing time: {emb_proc_time*1000:.4f} ms")
        if embedding is not None:
            print(f" Embedding Shape: {embedding.shape}, first 3 values: {embedding[:3]}")
        else:
            print(f" Embedding: FAILED")
        print("-" * 40)

    print("\n" + "=" * 80)
    print("✅ VoxSigil Hybrid Middleware Test Complete ✅")
    print("=" * 80)
    
    print("\nKey Production Considerations:")
    print("- External Configuration: Use .env files or environment variables (Pydantic-settings helps).")
    print("- Dependencies: Manage with requirements.txt or Poetry/PDM.")
    print("- Robust Component Stubs: Replace SigilPatchEncoder, VoxSigilRAG, BLTEnhancedRAG stubs with actual, battle-tested implementations.")
    print("- Asynchronous Operations: For IO-bound tasks (model inference), consider `async/await` if your framework supports it.")
    print("- Scalability: Design for horizontal scaling; ensure components are stateless or manage state carefully (e.g., distributed cache).")
    print("- Monitoring & Alerting: Integrate with systems like Prometheus/Grafana, Datadog. Log key metrics (latency, error rates, cache performance, route distribution).")
    print("- Comprehensive Testing: Unit tests for each class, integration tests for the middleware pipeline, and performance/load tests.")
    print("- Security: Sanitize inputs if they could be malicious, especially if context influences external calls or sensitive data access.")
