...
# Import must be at top-level for linters
try:
...
try:
    ...
    class BaseSettings:
        def __init__(self, **kwargs):
            # Set default values for the expected fields
...
# Import already moved to the top to satisfy style checkers
# from .sigil_patch_encoder import SigilPatchEncoder
...
class VoxSigilRAG:
    ...
    def create_rag_context(
        ...
        logger.info(
            f"StandardRAG: Creating context for query: '{query[:50]}...' with {num_sigils} sigils."
        )
        # Mock implementation
        # Compute the query embedding (reserved for future similarity search logic)
        _ = self._compute_text_embedding(query)  # noqa: F841
        sigils = [
            {
                "id": f"std_doc_{i}",
                "content": f"Standard relevant document {i} for '{query[:20]}'",
...
class ByteLatentTransformerEncoder:
    ...
    def __init__(self, base_embedding_model=None, patch_size=64, max_patches=16):
        ...
        logger.info(
            f"ByteLatentTransformerEncoder initialized with patch_size={patch_size}, max_patches={max_patches} (hybrid_blt.py)"
        )
    ...
    def encode(self, text: str) -> np.ndarray:
        ...
        h = hashlib.sha256(
            f"blt_{text}".encode()
        ).digest()  # Add prefix to differentiate
        return np.frombuffer(h, dtype=np.float32)[:128]  # Use first 128 floats
    ...
    def create_patches(self, text: str) -> List[Any]:
        ...
        try:
            ...
            patch = Patch(
                validated_text, 0, len(validated_text), 0.5
            )  # Default entropy
            return [patch]
        ...
        for i in range(0, text_len, self.patch_size):
            ...
            try:
                ...
                logger.warning(
                    f"Error calculating entropy for chunk at position {i}: {e}"
                )
    ...
    def calculate_similarity(
        ...
        if not isinstance(embedding1, np.ndarray) or not isinstance(
            embedding2, np.ndarray
        ):
            logger.warning(
                f"Invalid embedding types: {type(embedding1)}, {type(embedding2)}"
            )
            return 0.0
...
class BLTEnhancedRAG(VoxSigilRAG):  # Inherits for simplicity in stub
    ...
    def __init__(self, entropy_threshold: float, blt_hybrid_weight: float):
        ...
        logger.info(
            f"BLTEnhancedRAG initialized with threshold={entropy_threshold}, weight={blt_hybrid_weight} (stub)."
        )
    ...
    def create_rag_context(
        ...
        logger.info(
            f"BLTRAG: Creating context for query: '{query[:50]}...' with {num_sigils} sigils."
        )
        # Mock implementation using BLT specific logic potentially
        _ = self._compute_text_embedding(query)  # For similarity search
        sigils = [
            {
                "id": f"blt_doc_{i}",
...
class HybridMiddleware:
    ...
    def _normalize_sigil_relationships_format(self):
        ...
        """
        if (
            not self.voxsigil_rag
            or not hasattr(self.voxsigil_rag, "_loaded_sigils")
            or not self.voxsigil_rag._loaded_sigils
        ):
        ...
        if (
            not hasattr(self.voxsigil_rag, "_loaded_sigils")
            or not self.voxsigil_rag._loaded_sigils
        ):
    ...
    def _enhance_messages_with_context(
        ...
        # Prepare the system-level context to prepend
        system_message_content = context.strip()

        # If the first message is already a system message, prepend the new context
        if enhanced_messages and enhanced_messages[0].get("role") == "system":
            ...
            # Otherwise, insert a new system message containing the context
...
def hybrid_embedding_utility(
    ...
    try:
        ...
        if avg_entropy < config.entropy_threshold * 0.8:  # Clearly low entropy
            ...
            logger.debug(
                "Utility: Borderline entropy, computing true hybrid embedding."
            )
            ...
            embedding = hybrid_emb / (
                np.linalg.norm(hybrid_emb) + 1e-9
            )  # Normalize final
            method_used = "hybrid_weighted"

    except Exception as e:
        logger.error(
            f"Utility: Error in hybrid_embedding: {e}. Falling back to standard.",
            exc_info=True,
        )
        try:
            embedding = standard_encoder.encode(text)
            ...
            logger.critical(
                f"Utility: Standard embedding fallback also failed: {fallback_e}",
                exc_info=True,
            )
...
def entropy_router(text, config=None):
    ...
    patch_encoder = SigilPatchEncoder(
        entropy_threshold=config.entropy_threshold, use_blt=True
    )
...
if __name__ == "__main__":
    ...
    test_llm_requests = [
        {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "Explain how symbolic reasoning works in LLMs.",
                }
            ],
            "temperature": 0.5,
        },
        {
            ...
            "messages": [
                ...
                {
                    "role": "user",
                    "content": '{"sigil_id": "S001", "type": "metadata_query", "fields": ["name", "description"]}',
                },
            ],
        },
        {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "Using VoxSigil with <tag>BLT components</tag>, how can we implement entropy-based compression for text containing <code>my_function(a,b)</code>?",
                }
            ],
        },
        {  # Test cache
    ...
    for i, req_data in enumerate(test_llm_requests):
        ...
        print(
            f" Middleware processing time: {(end_req_time - start_req_time) * 1000:.2f} ms"
        )
        print(" Enhanced Messages (System context preview):")
        if (
            enhanced_req_data["messages"]
            and enhanced_req_data["messages"][0]["role"] == "system"
            and enhanced_req_data["messages"][0]["content"]
        ):
        ...
        print(
            f" VoxSigil Metadata: {metadata.get('route_method_used', 'N/A')}, Cache: {metadata.get('cache_hit', 'N/A')}"
        )
    ...
    for query in utility_queries:
        ...
        if embedding is not None:
            print(f" Embedding shape: {embedding.shape}, Preview: {embedding[:4]}...")
        else:
            print(" Embedding: FAILED")
        print("-" * 40)

    print("\n✅ VoxSigil Hybrid Middleware Test Complete ✅")
    print("=" * 80)

    print("\nKey Production Considerations:")
    print(
        "- External Configuration: Use .env files or environment variables (Pydantic-settings helps)."
    )
    print("- Dependencies: Manage with requirements.txt or Poetry/PDM.")
    print(
        "- Robust Component Stubs: Replace SigilPatchEncoder, VoxSigilRAG, BLTEnhancedRAG stubs with actual, battle-tested implementations."
    )
    print(
        "- Asynchronous Operations: For IO-bound tasks (model inference), consider `async/await` if your framework supports it."
    )
    print(
        "- Scalability: Design for horizontal scaling; ensure components are stateless or manage state carefully (e.g., distributed cache)."
    )
    print(
        "- Monitoring & Alerting: Integrate with systems like Prometheus/Grafana, Datadog. Log key metrics (latency, error rates, cache performance, route distribution)."
    )
    print(
        "- Comprehensive Testing: Unit tests for each class, integration tests for the middleware pipeline, and performance/load tests."
    )
    print(
        "- Security: Sanitize inputs if they could be malicious, especially if context influences external calls or sensitive data access."
    )
