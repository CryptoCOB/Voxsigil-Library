...
class JsonFileMemoryInterface(BaseMemoryInterface):
    ...
    def __init__(self, memory_dir: Optional[Union[str, Path]] = None):
        ...
        if memory_dir is None:
            ...
            self.memory_dir = (
                Path(__file__).resolve().parent.parent.parent / "voxsigil_memory"
            )
        else:
        ...
        self.logger.info(
            f"Initialized JsonFileMemoryInterface with directory: {self.memory_dir}"
        )
    ...
    def _ensure_index_file(self):
        ...
        if not self.index_file.exists():
            with open(self.index_file, "w") as f:
                json.dump(
                    {"interactions": [], "last_updated": datetime.now().isoformat()},
                    f,
                    indent=2,
                )
