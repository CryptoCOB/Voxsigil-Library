...
try:
    ...
    def validate(instance: Any, schema: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.warning("jsonschema package not installed - validation skipped")
...
# Define paths
DEFAULT_VOXSİGİL_LIBRARY_PATH = (
    Path(__file__).resolve().parent.parent / "voxsigil-Library"
)
DEFAULT_EMBEDDING_MODEL = "all-MiniLM-L6-v2"  # Common default, good for CPU
DEFAULT_EMBEDDINGS_CACHE_PATH = Path(
    "/kaggle/working/voxsigil_cache/embeddings_cache.npz"
)
...
class VoxSigilRAG:
    ...
    def __init__(
        ...
        embedding_dim: int = 128,  # Added embedding_dim parameter for BLT compatibility
        ...
        max_patch_size: int = 1024,  # Added max_patch_size parameter
    ):  # Approx 2000 tokens
        """
        Initialize the VoxSigil RAG handler.        Args:
            ...
            embedding_dim: Dimension of the embedding vectors (for BLT compatibility).
            ...
            max_patch_size: Maximum patch size for some operation (example).
        """
        self.voxsigil_library_path = (
            voxsigil_library_path or DEFAULT_VOXSİGİL_LIBRARY_PATH
        )
        ...
        self.default_max_context_chars = (
            default_max_context_chars  # Store max_patch_size
        )
        self.max_patch_size = max_patch_size

        # Store embedding_dim for BLT compatibility
        ...
        self._injected_methods: Dict[str, Callable] = {}

        logger.info("VoxSigilRAG initialized with extension framework support")

    def register_extension(self, name: str, extension: Any) -> None:
        """
        Register an extension with this RAG instance.

        Args:
            name: Name of the extension
            extension: Extension object with initialize() and extend_capabilities() methods
        """
        ...
        # Initialize extension if it has an initialize method
        if hasattr(extension, "initialize"):
            try:
                extension.initialize(self)
                logger.info(f"Extension {name} initialized")
            except Exception as e:
                logger.error(f"Failed to initialize extension {name}: {e}")

        # Add new methods if extension provides them
        if hasattr(extension, "extend_capabilities"):
            try:
                new_methods = extension.extend_capabilities(self)
                for method_name, method_func in new_methods.items():
                    self._injected_methods[method_name] = method_func
                    setattr(self, method_name, method_func)
                logger.info(f"Extension {name} added {len(new_methods)} new methods")
            except Exception as e:
                logger.error(f"Failed to extend capabilities for {name}: {e}")
    ...
    def unregister_extension(self, name: str) -> None:
        ...
        methods_to_remove = []
        for method_name, method_func in self._injected_methods.items():
        ...
        for method_name in methods_to_remove:
            ...
            if method_name in self._injected_methods:
                del self._injected_methods[method_name]
    ...
    def load_all_sigils(self, force_reload: bool = False) -> List[Dict[str, Any]]:
        ...
        for file_path in sigil_files:
            ...
            if sigil_content:
                ...
                if (
                    "_last_modified" not in sigil_content
                ):  # Should be added by _load_sigil_file
                    try:
                        sigil_content["_last_modified"] = file_path.stat().st_mtime
                    except FileNotFoundError:
                        logger.warning(
                            f"Could not stat file {file_path} during load_all_sigils fallback."
                        )
        ...
        if force_reload and HAVE_SENTENCE_TRANSFORMERS and self.embedding_model:
            logger.info(
                "Force reload: Consider running precompute_all_embeddings(force_recompute=True) if embeddings need update."
            )
        return self._loaded_sigils
    ...
    def _validate_sigil_data(self, sigil_data: Dict[str, Any], file_path: Path) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Schema validation failed for {file_path}: {str(e)} (at path: '{path_str}')"
            )
            return False
        except Exception as e_generic:
            logger.error(
                f"Generic error during schema validation for {file_path}: {e_generic}"
            )
            return False
    ...
    def _load_sigil_file(self, file_path: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not isinstance(sigil_data, dict):
                logger.warning(
                    f"File {file_path} did not parse into a dictionary. Content type: {type(sigil_data)}. Skipping."
                )
                return None

            # Normalize relationships format to ensure schema validation passes
            if "relationships" in sigil_data and not isinstance(
                sigil_data["relationships"], dict
            ):
                ...
                if isinstance(sigil_data["relationships"], list):
                    ...
                    logger.debug(
                        f"Converted relationships from list to dictionary in sigil: {file_path}"
                    )
                else:
                    # If it's not a list or dict, convert to a simple dict with a default key
                    sigil_data["relationships"] = {
                        "default": sigil_data["relationships"]
                    }
                    logger.debug(
                        f"Converted non-dict relationships to dictionary in sigil: {file_path}"
                    )

            # FEATURE 10: Sigil Schema Validation
            if not self._validate_sigil_data(sigil_data, file_path):
                logger.warning(
                    f"Skipping sigil from {file_path} due to schema validation errors."
                )
                return None

            sigil_data["_last_modified"] = (
                file_mod_time  # Store mod time for FEATURE 4 / FEATURE 7
            )
    ...
    def format_sigil_for_prompt(
        ...
        if detail_level.lower() == "full":
            ...
            if "prompt_template" in sigil and isinstance(
                sigil["prompt_template"], dict
            ):
    ...
    def _optimize_context_by_chars(
        ...
        try:
            ...
            current_detail_idx = (
                1  # Default to 'standard' if initial_detail_level is unknown
            )
    ...
    def _compute_text_embedding(self, text: str) -> np.ndarray:
        ...
        Args:
         ...
         Returns:
               Embedding vector (as a 1D numpy array)
        """
        ...
        if (
            hasattr(embedding, "detach")
            and hasattr(embedding, "cpu")
    ...
    def create_rag_context(
        ...
        hook_args = {
            ...
            "exclude_sigil_ids": exclude_sigil_ids,
        ...
        pre_hook_results = self.call_hooks("pre_create_rag_context", **hook_args)

        # Process any modifications from pre-hooks
        for result in pre_hook_results:
            if isinstance(result, dict):
                # Update parameters based on hook results
                query = result.get("query", query)
                num_sigils = result.get("num_sigils", num_sigils)
                filter_tags = result.get("filter_tags", filter_tags)
                # Add other parameters as needed
        all_loaded_sigils = self.load_all_sigils()
        if not all_loaded_sigils:
            logger.warning(
                "No sigils loaded from the library. RAG context will be empty."
            )
            return "", []
        ...
        norm_exclude_ids = (
            {sid.lower() for sid in exclude_sigil_ids} if exclude_sigil_ids else set()
        )
            ...
            if not is_excluded and norm_exclude_tags:
                ...
                if "tags" in sigil:
                    ...
                    if isinstance(s_tags_val, list):
                        current_s_tags_set.update(t.lower() for t in s_tags_val)
                    elif isinstance(s_tags_val, str):
                        current_s_tags_set.add(s_tags_val.lower())
                if "tag" in sigil and sigil["tag"]:  # legacy
                    current_s_tags_set.add(sigil["tag"].lower())
                if not norm_exclude_tags.isdisjoint(
                    current_s_tags_set
                ):  # if any exclusion tag is present
                    is_excluded = True
        ...
        logger.debug(
            f"Applied exclusion filters: {len(all_loaded_sigils) - len(current_sigils_pool)} excluded. {len(current_sigils_pool)} remaining."
        )

        # Apply inclusion tag filtering (if any sigils remain)
        if filter_tag and not filter_tags:
            filter_tags = [filter_tag]  # backward compatibility
        ...
        if filter_tags and current_sigils_pool:
            ...
            for sigil in current_sigils_pool:
                ...
                if tag_operator.upper() == "AND":
                    if norm_filter_tags.issubset(current_s_tags_set):
                        match = True
                else:  # OR (default)
                    if not norm_filter_tags.isdisjoint(current_s_tags_set):
                        match = True
            ...
            logger.debug(
                f"After inclusion tag filtering: {len(current_sigils_pool)} sigils."
            )

        # Semantic search if query is provided
        sigils_with_scores: List[
            Dict[str, Any]
        ] = []  # This will hold sigils with their scores
        ...
        if (
            ...
            logger.info(
                f"Performing semantic search with query: '{effective_query}' on {len(current_sigils_pool)} candidates."
            )
            ...
            logger.info(
                f"Semantic search yielded {len(sigils_with_scores)} sigils meeting threshold {min_score_threshold:.2f}."
            )
        elif (
            current_sigils_pool
        ):  # No semantic search, but sigils remain after filtering
            logger.info(
                f"No semantic search performed. Using {len(current_sigils_pool)} sigils from filtering stage. Assigning neutral score."
            )
            for sigil_data in current_sigils_pool:
                retrieved_item = sigil_data.copy()
                retrieved_item["_similarity_score"] = (
                    0.5  # Neutral score if only filtered
                )
                sigils_with_scores.append(retrieved_item)
        ...
        if (
            ...
            and len(selected_sigils_for_context) < num_sigils
        ):
            logger.debug(
                f"Selected {len(selected_sigils_for_context)} sigils, less than requested {num_sigils}. Attempting auto-fusion."
            )
            ...
            if len(fused_sigils) > len(selected_sigils_for_context):
                ...
                logger.info(
                    f"Auto-fused sigils. Total now: {len(selected_sigils_for_context)}"
                )
        ...
        if enable_context_optimizer and final_retrieved_sigils:
            ...
            logger.info(
                f"Applying dynamic context optimization with budget: {char_budget} chars."
            )
        ...
        for s_format_item in final_retrieved_sigils:
            formatted_text = self.format_sigil_for_prompt(
                s_format_item, current_detail_level
            )
        ...
        sigil_context_str = "\n\n---\n\n".join(
            formatted_sigils_parts
        )  # Use a clear separator        # Save embeddings cache periodically or at end of a significant operation
        if (
            self.cache_enabled
            and HAVE_SENTENCE_TRANSFORMERS
            and len(self._embeddings_cache) % 100 > 80
        ):  # Heuristic
            self._save_embeddings_cache()

        # Call post-hooks for extensions
        result = (sigil_context_str, final_retrieved_sigils)
        post_hook_results = self.call_hooks(
            "post_create_rag_context", result, **hook_args
        )

        # Process any modifications from post-hooks
        for post_result in post_hook_results:
            if isinstance(post_result, tuple) and len(post_result) == 2:
                result = post_result
                break
    ...
    def inject_voxsigil_context(
        ...
        """
        search_query = (
            query if query is not None else prompt
        )  # Use prompt as query if none specific provided
...
"""
    ...
    def create_sigil_index(
        self,
    ) -> Dict[str, Dict[str, Any]]:  # Renamed from create_cached_sigil_index
        """
        ...
        for sigil_item in sigils:
            if "sigil" in sigil_item and isinstance(sigil_item["sigil"], str):
                ...
                if sigil_id in sigil_index:
                    logger.warning(
                        f"Duplicate sigil ID '{sigil_id}' found. Overwriting with content from '{sigil_item.get('_source_file')}'. Original was '{sigil_index[sigil_id].get('_source_file')}'."
                    )
    ...
    def _get_sigil_embedding(self, sigil: Dict[str, Any]) -> Optional[np.ndarray]:
        ...
        if sigil_id_part:
            ...
            if source_file_part:
                cache_key_base = (
                    f"file:{source_file_part}"  # If ID missing but file known
                )
            else:  # Absolute fallback
                cache_key_base = (
                    f"hash:{hashlib.md5(str(sigil).encode('utf-8')).hexdigest()}"
                )
        ...
        if not text_to_embed.strip():
            logger.debug(
                f"No text content to embed for sigil: {sigil_id_part or sigil.get('_source_file', 'N/A')}"
            )
            return None
        ...
        try:
            ...
            logger.error(
                f"Error generating embedding for sigil ({sigil_id_part or 'N/A'}): {e}"
            )
            return None

    def _get_sigil_text_for_embedding(self, sigil: Dict[str, Any]) -> str:
        """
        Generate a text string from sigil data for embedding purposes.
        """
        texts = []
        if sigil.get("sigil"):
            texts.append(f"Sigil Name: {sigil['sigil']}")
        ...
        if sigil.get("tags"):
            ...
            if isinstance(tags_val, list):
                all_tags.extend(str(t) for t in tags_val)
            elif isinstance(tags_val, str):
                all_tags.append(tags_val)
        if all_tags:
            texts.append(f"Keywords: {', '.join(list(set(all_tags)))}")  # Unique tags

        if sigil.get("principle"):
            texts.append(f"Core Principle: {sigil['principle']}")
    ...
    def _load_embeddings_cache(self, file_path: Optional[Path] = None) -> None:
        ...
        if not path_to_load.exists():
            logger.info(
                f"Embeddings cache not found at {path_to_load}. Starting with an empty cache."
            )
        ...
        try:
            ...
            if (
                "embeddings_dict" in cache_data
                and cache_data["embeddings_dict"].size > 0
            ):
                loaded_cache = cache_data["embeddings_dict"].item()
                if isinstance(loaded_cache, dict):
                    self._embeddings_cache.update(
                        loaded_cache
                    )  # Merge, new values overwrite
                    logger.info(
                        f"Loaded {len(loaded_cache)} embeddings. Total cached: {len(self._embeddings_cache)}"
                    )
                else:
                    logger.warning(
                        f"Embeddings data in {path_to_load} is not a dictionary. Cache not loaded."
                    )
            elif "embeddings" in cache_data:  # Legacy support for old format
                legacy_embeddings = cache_data.get("embeddings")
                if (
                    hasattr(legacy_embeddings, "ndim")
                    and legacy_embeddings.ndim == 0
                    and isinstance(legacy_embeddings.item(), dict)
                ):
                    self._embeddings_cache.update(legacy_embeddings.item())
                    logger.info(
                        f"Loaded legacy format embeddings. Total cached: {len(self._embeddings_cache)}"
                    )
                else:
                    logger.warning(
                        "Found 'embeddings' key but not in expected 0-d array format. Cache not loaded from this key."
                    )
            else:
                logger.info(
                    f"No 'embeddings_dict' or compatible 'embeddings' key found in {path_to_load}. Cache remains as is."
                )
    ...
    def _save_embeddings_cache(self, file_path: Optional[Path] = None) -> None:
        ...
        if not self.cache_enabled or not self._embeddings_cache:
            if not self._embeddings_cache:
                logger.debug("Embeddings cache is empty. Nothing to save.")
            return

        try:
            path_to_save.parent.mkdir(parents=True, exist_ok=True)
            logger.info(
                f"Saving {len(self._embeddings_cache)} embeddings to cache at {path_to_save}"
            )
            # Save the dictionary as a 0-d object array for robust loading
            np.savez_compressed(
                path_to_save,
                embeddings_dict=np.array(
                    self._embeddings_cache, dtype=object
                ),  # Correct way to save dict
    ...
    def export_embeddings_cache(self, export_path: Path) -> bool:
        ...
        try:
            ...
            )  # This method now handles logging
            return True
    ...
    def import_embeddings_cache(self, import_path: Path, merge: bool = True) -> bool:
        ...
        try:
            ...
            if (
                ...
                loaded_entries = (
                    cache_data["embeddings"].item()
                    if hasattr(cache_data["embeddings"], "ndim")
                    and cache_data["embeddings"].ndim == 0
                    else {}
                )
            else:
                loaded_entries = {}
            ...
            if not isinstance(loaded_entries, dict):
                logger.error(
                    f"Data in {import_path} is not a dictionary. Import failed."
                )
                return False

            if merge:  # Current cache takes precedence if keys collide, or other way?
                # Standard update: imported takes precedence.
                # To have existing take precedence, we could create a copy of loaded_entries, update it with self._embeddings_cache, then assign it back
                self._embeddings_cache.update(loaded_entries)
                logger.info(
                    f"Merged {len(loaded_entries)} embeddings. Total now: {len(self._embeddings_cache)}."
                )
            else:
                self._embeddings_cache = loaded_entries
                logger.info(
                    f"Replaced cache with {len(loaded_entries)} embeddings from {import_path}."
                )
            return True
        except Exception as e:
    ...
    def precompute_all_embeddings(
        ...
        if not HAVE_SENTENCE_TRANSFORMERS or not self.embedding_model:
            logger.warning(
                "Sentence Transformers not available. Cannot precompute embeddings."
            )
            return 0

        all_sigils_list = self.load_all_sigils(
            force_reload=force_recompute
        )  # Force reload if recomputing all
        if not all_sigils_list:
            logger.info("No sigils loaded to precompute embeddings for.")
            return 0

        texts_to_embed_batch: List[str] = []
        sigil_cache_keys_batch: List[
            str
        ] = []  # To map embeddings back to their cache keys
        ...
        for sigil_data in all_sigils_list:
            ...
            if sigil_id_part:
                cache_key_base = f"id:{sigil_id_part}"
            elif sigil_data.get("_source_file"):
                cache_key_base = f"file:{sigil_data['_source_file']}"
            else:
                cache_key_base = (
                    f"hash:{hashlib.md5(str(sigil_data).encode('utf-8')).hexdigest()}"
                )
        ...
        if not texts_to_embed_batch:
            logger.info(
                "All sigil embeddings seem to be up-to-date in cache (or no text to embed). Force recompute if needed."
            )
            return 0

        logger.info(
            f"Starting batch embedding computation for {len(texts_to_embed_batch)} sigils."
        )
        computed_count = 0
        try:
            ...
            logger.info(
                f"Successfully computed and cached {computed_count} new/updated embeddings."
            )
        ...
        if sigil_id:
            if sigil_id not in self._sigil_retrieval_counts:
                return {
                    "id": sigil_id,
                    "retrieval_count": 0,
                    "last_retrieved_iso": None,
                    "message": "Sigil not found in analytics.",
                }
            return {
                "id": sigil_id,
                "retrieval_count": self._sigil_retrieval_counts.get(sigil_id, 0),
                "last_retrieved_iso": datetime.fromtimestamp(
                    self._sigil_last_retrieved_time[sigil_id], tz=timezone.utc
                ).isoformat()
                if sigil_id in self._sigil_last_retrieved_time
                else None,
            }
            ...
            return {
                "total_retrievals_tracked_for_session": sum(
                    self._sigil_retrieval_counts.values()
                ),
    ...
    def analyze_sigil_match(
        self, sigil: Dict[str, Any], query: str, similarity_score: float
    ) -> str:
        ...
        if "tag" in sigil and sigil["tag"]:
            sigil_tags_set.add(str(sigil["tag"]).lower())
        if "tags" in sigil:
            tags_val = sigil["tags"]
            if isinstance(tags_val, list):
                sigil_tags_set.update(str(t).lower() for t in tags_val)
            elif isinstance(tags_val, str):
                sigil_tags_set.add(tags_val.lower())

        matched_tags = query_words.intersection(sigil_tags_set)
        if matched_tags:
            explanation_parts.append(
                f"Query matched tags: {', '.join(sorted(list(matched_tags)))}"
            )
        ...
        if "principle" in sigil and sigil["principle"]:
            ...
            principle_matched_keywords = {
                qw for qw in query_words if qw in principle_lower
            }
            if principle_matched_keywords:
                explanation_parts.append(
                    f"Query terms in principle: {', '.join(sorted(list(principle_matched_keywords)))}"
                )
    ...
    def find_related_sigils(
        self,
        sigil_id: str,
        relationship_types: Optional[List[str]] = None,
        include_semantic_relatives: bool = True,
        num_semantic: int = 3,
        ...
        if not source_sigil_data:
            logger.warning(
                f"Source sigil '{sigil_id}' not found in index for finding relations."
            )
            return []
        ...
        if "relationships" in source_sigil_data and isinstance(
            source_sigil_data["relationships"], dict
        ):
        ...
        if (
            ...
            if source_embedding is not None:
                ...
                for other_id, other_data in sigil_index.items():
                    ...
                    if other_embedding is not None:
                        ...
                        if not np.isnan(sim) and sim >= semantic_threshold:
                            ...
                            s_match.update(
                                {
                                    "_relation_type": "semantic",
                                    "_relation_source": sigil_id,
                                    "_similarity_score": float(sim),
                                }
                            )
                ...
                for sem_match in semantic_candidates[:num_semantic]:
                    if (
                        sem_match["sigil"] not in found_related_ids
                    ):  # Double check ID before adding
...
if __name__ == "__main__":
    ...
    logger.info(f"Max patch size from singleton: {voxsigil_rag.max_patch_size}")
    ...
    if HAVE_SENTENCE_TRANSFORMERS:
        ...
        logger.warning(
            "Skipping batch embedding test as SentenceTransformers not available."
        )
    ...
    enhanced_prompt, retrieved_items = inject_voxsigil_context(
        ...
        exclude_tags=[
            "gamma"
        ],  # Exclude sigils with 'gamma' tag (should exclude GammaExcluded)
    ...
    if retrieved_items and retrieved_items[0].get("sigil"):
        ...
        print(
            json.dumps(
                voxsigil_rag.get_sigil_analytics(sigil_id=retrieved_items[0]["sigil"]),
                indent=2,
            )
        )
    ...
    if HAVE_SENTENCE_TRANSFORMERS:
        ...
        if voxsigil_rag.export_embeddings_cache(cache_export_file):
            ...
            new_rag_instance = VoxSigilRAG(
                voxsigil_library_path=test_lib_path,
                cache_enabled=True,
                max_patch_size=512,
            )
            logger.info(
                f"New instance max_patch_size: {new_rag_instance.max_patch_size}"
            )
            new_rag_instance._embeddings_cache = {}  # Clear initial cache
            logger.info(
                f"New instance initially has {len(new_rag_instance._embeddings_cache)} embeddings."
            )
            if new_rag_instance.import_embeddings_cache(cache_export_file):
                logger.info(
                    f"Successfully imported. New instance now has {len(new_rag_instance._embeddings_cache)} embeddings."
                )
    ...
    try:
        if sigil_alpha_file.exists():
            os.remove(sigil_alpha_file)
        if sigil_beta_file.exists():
            os.remove(sigil_beta_file)
        if sigil_gamma_file.exists():
            os.remove(sigil_gamma_file)
        ...
        logger.error(
            f"Error during cleanup: {e}. Manual cleanup of 'Temp-VoxSigil-Library' might be needed."
        )
