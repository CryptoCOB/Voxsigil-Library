...
class VoxSigilBLTMiddleware:
    ...
    def _initialize_middleware(self):
        ...
        try:
            ...
            logger.info(f"Initializing BLT middleware with entropy_threshold={entropy_threshold}, " 
            ...
            if self.voxsigil_library_path.exists():
                ...
                except Exception as e:                    logger.error(f"Error initializing BLT RAG: {e}")
            else:
    ...
    def generate_with_blt(self, prompt, max_length=200, temperature=0.7, 
        ...
        try:
            # First, attempt to use RAG if available
            if self.rag:
                try:
                    # Check if input should use BLT enhancement based on entropy
                    entropy_score = self.rag.patch_encoder.compute_average_entropy(prompt)
                    should_use_blt = entropy_score > self.blt_config.get('entropy_threshold', 0.3)
                    ...
                    if should_use_blt:
                        ...
                        if results:
                            ...
                            return response, "BLT-Enhanced RAG"
    ...
    def create_rag_context(self, query: str, num_results: int = 5) -> str:
        ...
        try:
            ...
            if self.rag_processor:
                context, sigils = self.rag_processor.create_rag_context(query=query, num_sigils=num_results)
