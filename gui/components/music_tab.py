...
# VoxSigil imports - Made optional
try:
...
try:
    ...
    logger.info("🎵 Music agents imported successfully!")
except ImportError as e:
    MusicComposerAgent = None
    CompositionRequest = None
    VoiceModulatorAgent = None
    VoiceModulationRequest = None
    MusicSenseAgent = None
    MUSIC_AGENTS_AVAILABLE = False
    logger.warning(f"Music agents import failed: {e}")
    ...
    def update_visualization(self):
        ...
        if len(self.audio_data) > 0:
            ...
            self.waveform_label.setText(
                f"Waveform - Duration: {duration:.2f}s, Energy: {energy:.3f}"
            )
...
class GenreSelectionWidget(QWidget):
    ...
    def __init__(
        self, genre_vocabulary: Dict[str, Any], favorites: Optional[Dict[str, int]] = None
    ):
        super().__init__()
    ...
    def setup_ui(self):
        ...
        self.favorites_check.stateChanged.connect(
            lambda _: self.on_category_changed(self.category_combo.currentText())
        )
...
class VoiceModulationWidget(QWidget):
    ...
    def browse_input_file(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Select Audio File", "", "Audio Files (*.wav *.mp3 *.flac *.m4a);;All Files (*)"
        )
        if file_path:
    ...
    def on_modulate_clicked(self):
        ...
        if not self.input_path_edit.text():
            QMessageBox.warning(self, "Input Required", "Please select an input audio file.")
            return
...
class MusicTabWidget(QWidget):
    ...
    def initialize_agents(self):
        ...
        try:
            if self.vanta_core and MUSIC_AGENTS_AVAILABLE:
            ...
            elif not MUSIC_AGENTS_AVAILABLE:
                self.log_output("⚠️ Music agents not available, running in simulation mode")
            else:
    ...
    async def modulate_voice_async(self, params: Dict[str, Any]):
        ...
        try:
            if self.voice_modulator:
                ...
                # Resolve and load the input audio file
                ...
                try:
                    ...
                    sample_audio, sample_rate = sf.read(str(input_path))
                    # Convert to mono if necessary
                    if sample_audio.ndim > 1:
                        sample_audio = sample_audio.mean(axis=1)
                except ImportError:
                    # Fallback WAV loader (16-bit PCM)
                    ...
                    with wave.open(str(input_path), "rb") as wf:
                        ...
                        data = struct.unpack(
                            "<" + fmt * wf.getnframes() * wf.getnchannels(), frames
                        )
                        sample_audio = np.array(data, dtype=np.float32)
                        # Normalise and convert to mono
                        sample_audio /= float(2 ** (8 * width - 1))
                        if wf.getnchannels() > 1:
                            sample_audio = sample_audio.reshape(-1, wf.getnchannels()).mean(axis=1)
                ...
                self.audio_viz.update_audio_data(audio, sample_rate)
                self.log_output(
                    f"✅ Voice modulation completed (simulation): {params['target_profile']}"
                )
