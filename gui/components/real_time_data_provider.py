...
class SystemMetricsProvider:
    ...
    def get_system_metrics(self) -> Dict[str, Any]:
        ...
        if PSUTIL_AVAILABLE:
            ...
            try:
                ...
                # Fallback for Windows or permission denied
                try:
                    disk = psutil.disk_usage("C:\\")
                except (OSError, PermissionError) as e:
                    # Create dummy disk data if all disk access fails
            ...
            network = psutil.net_io_counters()  # CPU core temperatures if available
            try:
            ...
            return {
                ...
                "disk_total_gb": 512.0,
                "disk_free_gb": 512.0
                * (1 - (65 + 10 * math.sin(day_cycle * math.pi)) / 100),
    ...
    def _get_gpu_metrics(self):
        ...
        try:
            ...
            gpu_memory_total = mem_info.total / (1024**2)  # MB            # Temperature
            try:
                gpu_temp = pynvml.nvmlDeviceGetTemperature(
                    handle, pynvml.NVML_TEMPERATURE_GPU
                )
            except (pynvml.NVMLError, AttributeError) as e:
                logger.debug(f"GPU temperature not available: {e}")
                gpu_temp = None
            except Exception as e:
                logger.warning(f"Unexpected error reading GPU temperature: {e}")
                gpu_temp = None

            return gpu_usage, gpu_memory_used, gpu_memory_total, gpu_temp

        except (ImportError, AttributeError) as e:
            logger.debug(f"NVIDIA-ML not available: {e}")
        except Exception as e:
            logger.warning(f"Unexpected error accessing GPU via NVIDIA-ML: {e}")
        ...
        if TORCH_AVAILABLE and torch.cuda.is_available():
            try:
                ...
                logger.warning(f"Unexpected error accessing GPU via PyTorch: {e}")
    ...
    def _get_vanta_simulation(self) -> Dict[str, Any]:
        ...
        if PSUTIL_AVAILABLE:
            ...
            return {
                ...
                "degraded_components": min(
                    1, active_components - (active_components - 1)
                ),
...
class AudioMetricsProvider:
    ...
    def _get_real_audio_level(self) -> float:
        ...
        try:
            ...
            audio_data = sd.rec(
                int(duration * 44100), samplerate=44100, channels=1, blocking=True
            )
            audio_level = float(
                np.abs(audio_data).mean() * 1000
            )  # Convert to dB-like scale
            return min(60.0, max(0.0, audio_level))  # Clamp to reasonable range

        except ImportError as e:
            logger.debug(f"Audio libraries not available: {e}")
        except (RuntimeError, OSError) as e:
            logger.debug(f"Audio device access failed: {e}")
        except Exception as e:
            logger.warning(f"Unexpected error reading audio level: {e}")
...
class RealTimeDataProvider:
    """
    Unified Real-Time Data Provider

    Main interface class that the GUI components use to access all real-time metrics.
    Aggregates data from all the individual provider classes.
    """

    def __init__(self):
        """Initialize the unified data provider."""
        self.system_provider = _system_provider
        self.vanta_provider = _vanta_provider
        self.training_provider = _training_provider
        self.audio_provider = _audio_provider

        logger.info("🔄 RealTimeDataProvider initialized with all metric sources")

    def get_system_metrics(self) -> Dict[str, Any]:
        """Get real-time system metrics."""
        return self.system_provider.get_system_metrics()

    def get_vanta_metrics(self) -> Dict[str, Any]:
        """Get real-time VantaCore metrics."""
        return self.vanta_provider.get_vanta_metrics()

    def get_training_metrics(self) -> Dict[str, Any]:
        """Get real-time training metrics."""
        return self.training_provider.get_training_metrics()

    def get_audio_metrics(self) -> Dict[str, Any]:
        """Get real-time audio metrics."""
        return self.audio_provider.get_audio_metrics()
    ...
    def get_all_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            all_metrics.update(self.get_vanta_metrics())
            all_metrics.update(self.get_training_metrics())
            all_metrics.update(self.get_audio_metrics())

            # Add metadata
            all_metrics["_provider_info"] = {
                "timestamp": time.time(),
                "sources": ["system", "vanta", "training", "audio"],
                "total_metrics": len(all_metrics),
            }

        except Exception as e:
            logger.error(f"Error aggregating metrics: {e}")
            # Return at least basic metrics on error
            all_metrics = {
                "error": str(e),
                "timestamp": time.time(),
                "cpu_usage": 0,
