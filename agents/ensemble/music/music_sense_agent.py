...
class MusicSenseAgent(BaseAgent):
    ...
    async def _initialize_music_models(self):
        ...
        try:
            ...
            try:
                from VoxSigilRag.hybrid_blt import ByteLatentTransformerEncoder
    ...
    async def _analyze_audio_chunk(self, audio_data: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            melody_data, lyrics = await self._extract_melody_and_lyrics(
                stems.get("vocals", audio_data)
            )
    ...
    async def _extract_melody_and_lyrics(
        self, vocal_audio: np.ndarray
    ...
    async def _create_music_embedding(
        self, melody: Dict, key: Dict, tempo: Dict, stems: Dict
    ) -> np.ndarray:
    ...
    def _fallback_tempo_detection(self, audio: np.ndarray) -> Dict[str, Any]:
        """Fallback tempo detection"""
        return {"bpm": 120, "beats": [], "meter": "4/4", "confidence": 0.0}
