...
try:
    ...
    # Fallback if UnifiedVantaCore is not available
    VantaCore = None
...
class SleepTimeComputeAgent(BaseAgent):
    ...
    async def _load_genre_vocabulary(self) -> None:
        ...
        try:
            ...
            if vocab_path.exists():
                ...
                logger.info("📚 Loaded genre vocabulary for sleep learning")
            else:
        ...
        for genre, preference in simulated_preferences.items():
            if genre in self.user_music_patterns["genre_preferences"]:
                ...
                self.user_music_patterns["genre_preferences"][genre] = (
                    1 - learning_rate
                ) * current + learning_rate * preference
            else:
    ...
    async def _analyze_temporal_patterns(self) -> None:
        ...
        # Determine current time-of-day segment for temporal pattern analysis
        ...
        logger.debug(f"Temporal pattern analysis: {current_period} (hour={time_of_day})")
        ...
        for period, genres in genre_time_associations.items():
            ...
            for genre in genres:
                current_strength = self.user_music_patterns["temporal_patterns"][period].get(
                    genre, 0.5
                )
                self.user_music_patterns["temporal_patterns"][period][genre] = min(
                    1.0, current_strength + 0.01
                )
    ...
    async def _analyze_mood_correlations(self) -> None:
        ...
        for mood, genres in mood_genre_correlations.items():
            ...
            for genre in genres:
                current_correlation = self.user_music_patterns["mood_correlations"][mood].get(
                    genre, 0.5
                )
                self.user_music_patterns["mood_correlations"][mood][genre] = min(
                    1.0, current_correlation + 0.005
                )
    ...
    async def _end_sleep_cycle(self) -> None:
        ...
        logger.info(
            f"✅ Sleep cycle completed: {self.current_cycle_metrics.genre_embeddings_updated} embeddings updated"
        )
