...
class AdaptiveMemoryManager(BaseAgent):
    ...
    async def async_init(self):
        ...
        self.logger.info(f"AdaptiveMemoryManager initialized with {available_memory:.1f}GB")
    ...
    async def _register_default_paradigms(self):
        ...
        paradigm_configs = [
            ParadigmConfig(
                ...
                target_memory_gb=self.budget.total_gb * self.budget.deltanet_percent / 100,
                max_latency_ms=50.0
            ),
            ParadigmConfig(
                name="minicache", 
                priority=MemoryPriority.CRITICAL,
                target_memory_gb=self.budget.total_gb * self.budget.minicache_percent / 100,
    ...
    async def register_paradigm(self, config: ParadigmConfig):
        ...
        self.logger.info(f"Registered paradigm: {config.name} "
    ...
    async def _collect_metrics(self) -> MemoryMetrics:
        ...
        if torch.cuda.is_available():
            ...
            metrics.gpu_total_gb = torch.cuda.get_device_properties(0).total_memory / (1024**3)
    ...
    async def _handle_high_usage(self, metrics: MemoryMetrics):
        ...
        self.logger.warning(f"High memory usage detected: {metrics.gpu_utilization:.1%}")
    ...
    async def _handle_critical_usage(self, metrics: MemoryMetrics):
        ...
        self.logger.error(f"Critical memory usage detected: {metrics.gpu_utilization:.1%}")
    ...
    async def _handle_fragmentation(self, metrics: MemoryMetrics):
        ...
        self.logger.info(f"Memory fragmentation detected: {metrics.fragmentation_ratio:.1%}")
    ...
    async def _compress_paradigm(self, paradigm_name: str):
        ...
        for callback in callbacks:
            try:
                ...
                self.logger.error(f"Error in compression callback for {paradigm_name}: {e}")
    ...
    async def _suspend_paradigm(self, paradigm_name: str):
        ...
        for callback in callbacks:
            try:
                ...
                self.logger.error(f"Error in suspension callback for {paradigm_name}: {e}")
    ...
    async def _update_cognitive_metrics(self, metrics: MemoryMetrics):
        ...
        if metrics.gpu_total_gb > 0:
            self.cognitive_metrics["memory_efficiency"] = min(
                metrics.gpu_utilization / self.alert_thresholds["high_usage"], 
                1.0
            )
        
        # Allocation success rate
        recent_failures = sum(1 for m in self.metrics_history[-60:] 
                            if m.allocation_failures > 0)
        ...
        if metrics.paradigm_usage:
            ...
            if usage_values:
                usage_variance = sum((x - sum(usage_values)/len(usage_values))**2 
                                   for x in usage_values) / len(usage_values)
                self.cognitive_metrics["paradigm_balance"] = max(0.0, 1.0 - usage_variance)
        
        # Degradation frequency: how often we need to degrade performance
        recent_degradations = sum(1 for m in self.metrics_history[-300:] 
                                if m.compaction_events > 0)
        self.cognitive_metrics["degradation_frequency"] = recent_degradations / 300.0
    
    async def allocate_paradigm_memory(self, paradigm_name: str, 
                                     size_gb: float) -> Optional[str]:
        ...
        if allocation_start is not None:
            self.logger.debug(f"Allocated {size_gb:.1f}GB for {paradigm_name} "
                            f"in {allocation_time:.1f}ms")
            return block_id
        else:
            self.logger.warning(f"Failed to allocate {size_gb:.1f}GB for {paradigm_name}")
            return None
    ...
    async def deallocate_paradigm_memory(self, block_id: str) -> bool:
        ...
        if success:
            self.logger.debug(f"Deallocated block {block_id} in {deallocation_time:.1f}ms")
    ...
    async def get_memory_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance": {
                "avg_allocation_time_ms": sum(self.allocation_times[-100:]) / len(self.allocation_times[-100:]) if self.allocation_times else 0.0,
                "avg_compaction_time_ms": sum(self.compaction_times[-10:]) / len(self.compaction_times[-10:]) if self.compaction_times else 0.0
            }
        }
    ...
    async def get_cognitive_load(self) -> float:
        ...
        recent_degradations = sum(1 for m in self.metrics_history[-60:] 
                                if m.compaction_events > 0)
    ...
    async def load_configuration(self, filepath: Path):
        ...
        if budget_data:
            ...
            self.budget.reserve_gb = budget_data.get("reserve_gb", self.budget.reserve_gb)
