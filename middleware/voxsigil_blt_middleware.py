...
class BLTEnhancedMiddleware(VoxSigilMiddleware):
    ...
    def __init__(
        ...
        self.blt_encoder = ByteLatentTransformerEncoder(entropy_threshold=entropy_threshold)
        ...
        self.patch_encoder = SigilPatchEncoder(
            base_embedding_model=base_model, entropy_threshold=entropy_threshold
        )
    ...
    def _blt_enhance_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if user_message:
            ...
            high_entropy_count = sum(
                1 for _, e in patches if e > self.blt_encoder.entropy_threshold
            )
    ...
    def _compress_content_with_blt(self, content: str, max_size: Optional[int] = None) -> str:
        ...
        logger.debug(
            f"BLT compression: {len(content)} → {len(compressed)} chars ({ratio:.2f} ratio)"
        )
    ...
    def wrap_llm_api(self, llm_api_call: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def blt_enhanced_wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if model_config and messages:
                logger.debug(
                    f"BLT-enhanced request for model {model_config.get('name', 'unknown')}"
                )
                ...
                if user_content and self.enable_patch_compression:
                    ...
                    avg_entropy = (
                        sum(entropy for _, entropy in patches) / len(patches) if patches else 0
                    )
    ...
    def _adaptive_compress(self, content: str, context_size: int = 0) -> str:
        ...
        logger.debug(
            f"Adaptive compression ({compression_level}): {content_length} → {len(encoded)} chars"
        )
    ...
    def create_context_manager(self, max_tokens: int = None) -> "BLTContextManager":
        ...
        """
        return BLTContextManager(middleware=self, max_tokens=max_tokens or self.max_context_size)
    ...
    def get_compression_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "encoder_stats": self.blt_encoder.get_stats()
            if hasattr(self.blt_encoder, "get_stats")
            else {},
        }
...
class BLTContextManager:
    ...
    def get_context(self) -> List[Dict[str, Any]]:
        ...
        if self.compressed_context:
            return [{"role": "system", "content": self.compressed_context}] + self.context_history[
                -2:
            ]
    ...
    def _optimize_context(self) -> None:
        ...
        if older_messages:
            ...
            self.compressed_context = self.middleware._adaptive_compress(
                combined_content, context_size=self._estimate_token_count()
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "compressed_context_size": len(self.compressed_context)
            if self.compressed_context
            else 0,
        }
