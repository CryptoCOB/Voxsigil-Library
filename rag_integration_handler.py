    ...
    def initialize_rag_interface(
        ...
        if interface_type == "supervisor":
            if VOXSIGIL_RAG_AVAILABLE:
                ...
                logger.warning(
                    "VoxSigilRAG not available, falling back to MockRagInterface"
                )
                self.rag_interface = MockRagInterface()
            ...
            if rag_processor:
                ...
                logger.warning(
                    "No RAG processor provided for SimpleRagInterface, falling back to MockRagInterface"
                )
                self.rag_interface = MockRagInterface()
                interface_type = "mock"  # Update type for correct metadata

        elif interface_type == "mock":
            logger.info("Initializing MockRagInterface")
            self.rag_interface = MockRagInterface()

        else:
            logger.warning(
                f"Unknown RAG interface type '{interface_type}', using MockRagInterface"
            )
            self.rag_interface = MockRagInterface()
            interface_type = "mock"  # Update type for correct metadata
