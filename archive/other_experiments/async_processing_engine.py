"""
Async Processing and Inference Engine for Nebula
...
from vanta_echo_nebula_bus import AsyncMessage, MessageType
logger = logging.getLogger("Nebula.AsyncProcessor")
...
class AsyncProcessingEngine:
    ...
    def __init__(self, vanta_echo_nebula_core: Any, config: ProcessorConfig):
        ...
        logger.info(f"{self.COMPONENT_NAME} registered with NebulaCore")

        # === NebulaCore Async Bus Integration ===
        # Register with NebulaCore's async bus and subscribe handler
        if hasattr(self.vanta_echo_nebula_core, "async_bus"):
            self.vanta_echo_nebula_core.async_bus.register_component("processing_engine")
            self.vanta_echo_nebula_core.async_bus.subscribe(
                "processing_engine",
            ...
            logger.warning(
                "NebulaCore async bus not available; async bus integration skipped."
            )

    async def handle_processing_request(self, message):
        """
        Async bus handler for PROCESSING_REQUEST messages.
        Use this method for all async processing requests via NebulaCore's async bus.
        Args:
    ...
    def _update_task_stats(
        ...
        with self._stats_lock:
            ...
            if (
                ...
                # Track timeout duration separately for better metrics
                detail.setdefault("total_timeout_time", 0.0)
                detail.setdefault("count_timeout", 0)
                detail["total_timeout_time"] += duration
                detail["count_timeout"] += 1
    ...
    async def get_stats(self) -> Dict:
        ...
        with self._stats_lock:  # Ensure consistent read of stats
            ...
            for task_type, details in self._stats["task_details"].items():
                ...
                if details.get("count_timeout", 0) > 0:
                    details["avg_timeout_seconds"] = (
                        details.get("total_timeout_time", 0.0) / details["count_timeout"]
                    )
                else:
                    details["avg_timeout_seconds"] = 0
    ...
    async def shutdown(self, graceful_timeout: float = 10.0):
        ...
        if self.worker_tasks:
            ...
            from utils.async_tools import gather_safe
            results = await gather_safe(self.worker_tasks)
...
# Example Usage (Conceptual - requires a mock vanta_echo_nebula_core and actual models)
async def main():
    # Mock NebulaCore for event publishing
    class MockNebulaCore:
        def publish_event(self, event_name: str, data: Dict, source: str):
            print(f"EVENT: [{source}] {event_name} -> {data}")

    vanta_echo_nebula_core_mock = MockNebulaCore()

    # Configure logger for Nebula.AsyncProcessor
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    )
    # You might want to set a more specific logger for the engine if it's noisy at INFO
    # logging.getLogger("Nebula.AsyncProcessor").setLevel(logging.DEBUG)
    ...
    engine = AsyncProcessingEngine(vanta_echo_nebula_core_mock, config)
    ...
    try:
        ...
        if HAVE_TRANSFORMERS and HAVE_TORCH:
            ...
            try:
                ...
                embeddings = await engine.get_embeddings(
                    ...
                    texts=["Hello world from VantaEchoNebula!", "Async processing is cool."],
                    priority=3,
                )
                print(
        ...
        custom_async_result = await engine.execute_custom(
            my_async_task, args=["Async Nebula"], priority=4
        )
...
if __name__ == "__main__":
    ...
    # This main() is for demonstration; real usage would involve a persistent NebulaCore.
