...
def log_event(event_name: str, metadata: Optional[Dict[str, Any]] = None, 
              level: str = "INFO", include_context: bool = True, exc_info=None) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        include_context: Whether to include thread-local context
        exc_info: Exception info to include (can be an exception, True to use sys.exc_info, or None)
    ...
    if not _check_rate_limit(event_name):
        return metadata  # Skip this event due to rate limiting
    
    # Create event data structure
    event_data = {
        "event": event_name,
        "timestamp": time.time(),
        "thread_info": _get_current_thread_info(),
        **metadata
    }
    ...
    if include_context:
        try:
            ...
            # Don't let context errors block logging
            event_data["context_error"] = str(e)
    ...
    if exc_info:
        ...
        if isinstance(exc_info, tuple) and len(exc_info) == 3:
            ...
            event_data["exception"] = {
                "type": str(exc_info[0].__name__ if exc_info[0] else ""),
                "message": str(exc_info[1]),
                "traceback": traceback.format_exception(*exc_info)
            }
        elif isinstance(exc_info, Exception):
            # Direct exception object
            event_data["exception"] = _format_exception(exc_info)
    
    # Format log message
    log_message = _format_log_message(event_name, event_data)
