...
class SimpleEvaluator:
    ...
    def evaluate(
        ...
        if not isinstance(response_text, str):
            self.logger.warning(
                f"Expected string for response but got {type(response_text)}. Converting to string."
            )
            try:
                response_text = str(response_text)
            except Exception as e:
                self.logger.error(
                    f"Failed to convert {type(response_text)} to string: {e}"
                )
                response_text = ""
        ...
        # Check term overlap
        if query_terms:
            term_overlap = len(query_terms.intersection(response_terms)) / len(
                query_terms
            )
        else:
            term_overlap = 0.0
        ...
        structure_score = (
            sum([has_paragraphs, has_bullet_points, has_numbered_list, has_sections])
            / 4.0
        )
        ...
        length_score = min(
            1.0, response_length / 500
        )  # Consider responses up to 500 chars

        # Final weighted score
        quality_score = (
            0.5 * relevance_score + 0.3 * structure_score + 0.2 * length_score
        )
    ...
    def _extract_keywords(self, text: str) -> List[str]:
        ...
        if not isinstance(text, str):
            self.logger.warning(
                f"Expected string for keyword extraction but got {type(text)}. Converting to string."
            )
            try:
                text = str(text)
            except Exception as e:
...
VoxSigilResponseEvaluator = None
VoxSigilConfig = None


def _load_voxsigil_evaluator():
    """Lazy load VoxSigil evaluator to avoid blocking imports"""
    global VOXSIGIL_EVALUATOR_AVAILABLE, VoxSigilResponseEvaluator, VoxSigilConfig
    if not VOXSIGIL_EVALUATOR_AVAILABLE:
        try:
            # Try to import from VoxSigilRag package
            from VoxSigilRag.voxsigil_evaluator import (
                VoxSigilConfig,
            ...
            logger_evaluation.info(
                "Successfully imported VoxSigilResponseEvaluator from VoxSigilRag package"
            )
        except ImportError:
            ...
            try:
                ...
                if parent_dir.exists():
                    ...
                    try:
                        ...
                        VOXSIGIL_EVALUATOR_AVAILABLE = True
                        logger_evaluation.info(
                            "Successfully imported VoxSigilResponseEvaluator from parent directory"
                        )
                    except ImportError as e:
                        logger_evaluation.error(
                            f"Failed to import VoxSigilResponseEvaluator: {e}"
                        )
            except Exception as e:
                logger_evaluation.error(
                    f"Failed to import VoxSigilResponseEvaluator: {e}"
                )
        return VOXSIGIL_EVALUATOR_AVAILABLE


# Don't load at import time - will be loaded when needed
# _load_voxsigil_evaluator()
...
class ResponseEvaluator:
    ...
    def __init__(self):
        ...
        self.external_evaluator = (
            None  # Try to initialize VoxSigilResponseEvaluator if available
        )
        if _load_voxsigil_evaluator():
            try:
                self.external_evaluator = VoxSigilResponseEvaluator()
                self.logger.info("Successfully initialized VoxSigilResponseEvaluator")
            except Exception as e:
                self.logger.error(
                    f"Failed to initialize VoxSigilResponseEvaluator: {e}"
                )
        else:
            self.logger.warning(
                "VoxSigilResponseEvaluator not available, using basic evaluation only"
            )
    ...
    def evaluate_response(
        ...
        if self.external_evaluator:
            try:
                external_results = self._external_evaluation(
                    query, response, context, expected_answer
                )
                # Merge with our basic results, with external results taking precedence
                results.update(external_results)
                self.logger.info("Used VoxSigilResponseEvaluator to evaluate response")
            except Exception as e:
                self.logger.error(
                    f"Error using VoxSigilResponseEvaluator for evaluation: {e}"
                )
    ...
    def _basic_evaluation(
        ...
        if response_length < min_expected_length:
            ...
            results["length_assessment"] = {
                "score": 0.6,
                "reason": "Response excessively long",
            }
        else:
            results["length_assessment"] = {
                "score": 1.0,
                "reason": "Response length appropriate",
            }
        ...
        if query_terms:
            overlap_ratio = len(query_terms.intersection(response_terms)) / len(
                query_terms
            )
            if overlap_ratio < 0.3:
                results["relevance_assessment"] = {
                    "score": 0.4,
                    "reason": "Low term overlap with query",
                }
            elif overlap_ratio < 0.6:
                results["relevance_assessment"] = {
                    "score": 0.7,
                    "reason": "Moderate term overlap with query",
                }
            else:
                results["relevance_assessment"] = {
                    "score": 0.9,
                    "reason": "High term overlap with query",
                }
        else:
            results["relevance_assessment"] = {
                "score": 0.5,
                "reason": "Unable to assess term overlap",
            }

        # Check for specificity (avoid vague responses)
        if re.search(
            r"\b(depends|various|several|many|different|etc\.)\b", response.lower()
        ):
            specificity_score = (
                0.7  # Potential indicator of generality, but not definitive
            )
        else:
            specificity_score = 0.9

        results["specificity_assessment"] = {
            "score": specificity_score,
            "reason": "Based on specificity indicators",
        ...
        if has_sections:
            results["structure_assessment"] = {
                "score": 1.0,
                "reason": "Well-structured with clear sections",
            }
        elif has_paragraphs:
            results["structure_assessment"] = {
                "score": 0.8,
                "reason": "Contains paragraph breaks",
            }
        else:
            results["structure_assessment"] = {
                "score": 0.5,
                "reason": "Limited structure or formatting",
            }
    ...
    def _external_evaluation(
        ...
        """
        Performs evaluation using the external VoxSigilResponseEvaluator.
        ...
        eval_inputs = {"query": query, "response": response}
            ...
            try:
                if "quality_score" in eval_result:
                    results["quality_assessment"] = {
                        ...
                        "reason": eval_result.get(
                            "quality_reason", "External evaluator assessment"
                        ),
                    }
                ...
                if "relevance_score" in eval_result:
                    results["relevance_assessment_ext"] = {
                        ...
                        "reason": eval_result.get(
                            "relevance_reason", "External evaluator assessment"
                        ),
                    }
                ...
                if "correctness_score" in eval_result:
                    results["correctness_assessment"] = {
                        ...
                        "reason": eval_result.get(
                            "correctness_reason", "External evaluator assessment"
                        ),
                    }

                # Add any overall score if provided
                if "overall_score" in eval_result:
                    results["external_overall_score"] = float(
    ...
    def _generate_evaluation_summary(self, results: Dict[str, Any]) -> str:
        ...
        if success:
            ...
            quality_level = (
                "poor" if overall_score < 0.5 else "below acceptable standards"
            )
            summary = f"Response is {quality_level} with an overall score of {overall_score:.2f}."
